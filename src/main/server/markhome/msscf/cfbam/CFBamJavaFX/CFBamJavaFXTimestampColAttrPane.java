// Description: Java 11 JavaFX Attribute Pane implementation for TimestampCol.
...
/**
 *	CFBamJavaFXTimestampColAttrPane JavaFX Attribute Pane implementation
 *	for TimestampCol.
 */
public class CFBamJavaFXTimestampColAttrPane
extends CFGridPane
implements ICFBamJavaFXTimestampColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTimestampColObj cur = getJavaFXFocusAsTimestampCol();
				if( cur != null ) {
					ICFBamTimestampColEditObj editObj = (ICFBamTimestampColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTimestampColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTimestampColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimestampColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTimestampCol( argFocus );
	...
	public ICFBamTimestampColObj getEffJavaFXFocus() {
		ICFBamTimestampColObj eff = (ICFBamTimestampColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTimestampColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTimestampColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTimestampColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTimestampColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTimestampColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTimestampColObj)editObj.create();
