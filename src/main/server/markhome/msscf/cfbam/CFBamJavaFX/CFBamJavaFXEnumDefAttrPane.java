// Description: Java 11 JavaFX Attribute Pane implementation for EnumDef.
...
/**
 *	CFBamJavaFXEnumDefAttrPane JavaFX Attribute Pane implementation
 *	for EnumDef.
 */
public class CFBamJavaFXEnumDefAttrPane
extends CFGridPane
implements ICFBamJavaFXEnumDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamEnumDefObj cur = getJavaFXFocusAsEnumDef();
				if( cur != null ) {
					ICFBamEnumDefEditObj editObj = (ICFBamEnumDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamEnumDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXEnumDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamEnumDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamEnumDefObj getEffJavaFXFocus() {
		ICFBamEnumDefObj eff = (ICFBamEnumDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamEnumDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamEnumDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamEnumDefObj focus = getJavaFXFocusAsEnumDef();
		ICFBamEnumDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamEnumDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamEnumDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamEnumDefObj)editObj.create();
