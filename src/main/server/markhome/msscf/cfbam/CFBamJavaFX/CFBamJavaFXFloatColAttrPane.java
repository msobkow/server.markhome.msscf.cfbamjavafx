// Description: Java 11 JavaFX Attribute Pane implementation for FloatCol.
...
/**
 *	CFBamJavaFXFloatColAttrPane JavaFX Attribute Pane implementation
 *	for FloatCol.
 */
public class CFBamJavaFXFloatColAttrPane
extends CFGridPane
implements ICFBamJavaFXFloatColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamFloatColObj cur = getJavaFXFocusAsFloatCol();
				if( cur != null ) {
					ICFBamFloatColEditObj editObj = (ICFBamFloatColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamFloatColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXFloatColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamFloatColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamFloatColObj getEffJavaFXFocus() {
		ICFBamFloatColObj eff = (ICFBamFloatColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamFloatColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamFloatColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamFloatColObj focus = getJavaFXFocusAsFloatCol();
		ICFBamFloatColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamFloatColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamFloatColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamFloatColObj)editObj.create();
