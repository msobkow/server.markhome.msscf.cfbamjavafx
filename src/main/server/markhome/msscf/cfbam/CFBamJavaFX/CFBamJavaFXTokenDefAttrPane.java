// Description: Java 11 JavaFX Attribute Pane implementation for TokenDef.
...
/**
 *	CFBamJavaFXTokenDefAttrPane JavaFX Attribute Pane implementation
 *	for TokenDef.
 */
public class CFBamJavaFXTokenDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTokenDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTokenDefObj cur = getJavaFXFocusAsTokenDef();
				if( cur != null ) {
					ICFBamTokenDefEditObj editObj = (ICFBamTokenDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTokenDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTokenDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTokenDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTokenDefObj getEffJavaFXFocus() {
		ICFBamTokenDefObj eff = (ICFBamTokenDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTokenDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTokenDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTokenDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTokenDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTokenDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTokenDefObj)editObj.create();
