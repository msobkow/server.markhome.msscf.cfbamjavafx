// Description: Java 11 JavaFX Attribute Pane implementation for UuidGen.
...
/**
 *	CFBamJavaFXUuidGenAttrPane JavaFX Attribute Pane implementation
 *	for UuidGen.
 */
public class CFBamJavaFXUuidGenAttrPane
extends CFGridPane
implements ICFBamJavaFXUuidGenPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuidGenObj cur = getJavaFXFocusAsUuidGen();
				if( cur != null ) {
					ICFBamUuidGenEditObj editObj = (ICFBamUuidGenEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuidGenObj focus = getEffJavaFXFocus();
		...
		extends CFInt16Editor
	{
		public SliceEditor() {
			super();
			setMinValue( CFBamUuidGenBuff.SLICE_MIN_VALUE );
			setMaxValue( CFBamUuidGenBuff.SLICE_MAX_VALUE );
			setFieldName( "Slice" );
		}
	}
		...
		extends CFInt32Editor
	{
		public BlockSizeEditor() {
			super();
			setMinValue( CFBamUuidGenBuff.BLOCKSIZE_MIN_VALUE );
			setMaxValue( CFBamUuidGenBuff.BLOCKSIZE_MAX_VALUE );
	...
	public CFBamJavaFXUuidGenAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidGenObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamUuidGenObj getEffJavaFXFocus() {
		ICFBamUuidGenObj eff = (ICFBamUuidGenObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuidGenObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuidGenObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setUuidValue( null );
		}
		else {
			getJavaFXEditorInitValue().setUuidValue( popObj.getOptionalInitValue() );
		}
	...
	public void postFields()
	{
		...
		ICFBamUuidGenObj focus = getJavaFXFocusAsUuidGen();
		ICFBamUuidGenEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuidGenEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getUuidValue() );
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuidGenEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuidGenObj)editObj.create();
