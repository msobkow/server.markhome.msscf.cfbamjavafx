// Description: Java 11 JavaFX Attribute Pane implementation for NumberType.
...
/**
 *	CFBamJavaFXNumberTypeAttrPane JavaFX Attribute Pane implementation
 *	for NumberType.
 */
public class CFBamJavaFXNumberTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXNumberTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNumberTypeObj cur = getJavaFXFocusAsNumberType();
				if( cur != null ) {
					ICFBamNumberTypeEditObj editObj = (ICFBamNumberTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNumberTypeObj focus = getEffJavaFXFocus();
	...
	protected class DigitsCFLabel
		extends CFLabel
	{
		public DigitsCFLabel() {
			super();
			setText( "Digits" );
		}
	}

	protected class DigitsEditor
		extends CFInt16Editor
	{
		public DigitsEditor() {
			super();
			setMinValue( CFBamNumberDefBuff.DIGITS_MIN_VALUE );
			setMaxValue( CFBamNumberDefBuff.DIGITS_MAX_VALUE );
			setFieldName( "Digits" );
		}
	}
		...
		extends CFInt16Editor
	{
		public PrecisEditor() {
			super();
			setMinValue( CFBamNumberDefBuff.PRECIS_MIN_VALUE );
			setMaxValue( CFBamNumberDefBuff.PRECIS_MAX_VALUE );
	...
	protected DigitsCFLabel javafxLabelDigits = null;
	protected DigitsEditor javafxEditorDigits = null;
	...
	public CFBamJavaFXNumberTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNumberTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsNumberType( argFocus );
		...
		label = getJavaFXLabelDigits();
		setHalignment( label, HPos.LEFT );
		setValignment( label, VPos.BOTTOM );
		add( label, 0, gridRow );
		gridRow ++;

		ctrl = getJavaFXEditorDigits();
		setHalignment( ctrl, HPos.LEFT );
		add( ctrl, 0, gridRow );
		gridRow ++;
	...
	public ICFBamNumberTypeObj getEffJavaFXFocus() {
		ICFBamNumberTypeObj eff = (ICFBamNumberTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNumberTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public DigitsCFLabel getJavaFXLabelDigits() {
		if( javafxLabelDigits == null ) {
			javafxLabelDigits = new DigitsCFLabel();
		}
		return( javafxLabelDigits );
	}

	public void setJavaFXLabelDigits( DigitsCFLabel value ) {
		javafxLabelDigits = value;
	}

	public DigitsEditor getJavaFXEditorDigits() {
		if( javafxEditorDigits == null ) {
			javafxEditorDigits = new DigitsEditor();
		}
		return( javafxEditorDigits );
	}

	public void setJavaFXEditorDigits( DigitsEditor value ) {
		javafxEditorDigits = value;
	}
	...
	{
		ICFBamNumberTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorDigits().setInt16Value( null );
		}
		else {
			getJavaFXEditorDigits().setInt16Value( popObj.getRequiredDigits() );
		}
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setNumberValue( null );
		}
		else {
			getJavaFXEditorInitValue().setNumberValue( popObj.getOptionalInitValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMinValue().setNumberValue( null );
		}
		else {
			getJavaFXEditorMinValue().setNumberValue( popObj.getOptionalMinValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMaxValue().setNumberValue( null );
		}
		else {
			getJavaFXEditorMaxValue().setNumberValue( popObj.getOptionalMaxValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamNumberTypeObj focus = getJavaFXFocusAsNumberType();
		ICFBamNumberTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNumberTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setRequiredDigits( getJavaFXEditorDigits().getInt16Value() );

		editObj.setRequiredPrecis( getJavaFXEditorPrecis().getInt16Value() );

		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getNumberValue() );

		editObj.setOptionalMinValue( getJavaFXEditorMinValue().getNumberValue() );

		editObj.setOptionalMaxValue( getJavaFXEditorMaxValue().getNumberValue() );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNumberTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNumberTypeObj)editObj.create();
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxEditorDigits != null ) {
			javafxEditorDigits.setDisable( ! isEditing );
		}
		if( javafxEditorPrecis != null ) {
