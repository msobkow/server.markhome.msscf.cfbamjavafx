// Description: Java 11 JavaFX View/Edit Form implementation for BlobCol.
...
/**
 *	CFBamJavaFXBlobColViewEditForm JavaFX View/Edit Form implementation
 *	for BlobCol.
 */
public class CFBamJavaFXBlobColViewEditForm
extends CFBorderPane
implements ICFBamJavaFXBlobColPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit BlobCol";
	...
	public CFBamJavaFXBlobColViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBlobColObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamBlobColObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getBlobColFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamBlobColEditObj editObj = (ICFBamBlobColEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXBlobColPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamBlobColObj argFocus = (ICFBamBlobColObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamBlobColObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsBlobCol() != null ) {
							if( null == getJavaFXFocusAsBlobCol().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXBlobColPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXBlobColPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
