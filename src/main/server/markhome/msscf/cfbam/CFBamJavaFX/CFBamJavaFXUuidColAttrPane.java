// Description: Java 11 JavaFX Attribute Pane implementation for UuidCol.
...
/**
 *	CFBamJavaFXUuidColAttrPane JavaFX Attribute Pane implementation
 *	for UuidCol.
 */
public class CFBamJavaFXUuidColAttrPane
extends CFGridPane
implements ICFBamJavaFXUuidColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuidColObj cur = getJavaFXFocusAsUuidCol();
				if( cur != null ) {
					ICFBamUuidColEditObj editObj = (ICFBamUuidColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuidColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuidColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsUuidCol( argFocus );
	...
	public ICFBamUuidColObj getEffJavaFXFocus() {
		ICFBamUuidColObj eff = (ICFBamUuidColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuidColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuidColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUuidColObj focus = getJavaFXFocusAsUuidCol();
		ICFBamUuidColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuidColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuidColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuidColObj)editObj.create();
