// Description: Java 11 JavaFX View/Edit Form implementation for Value.
...
/**
 *	CFBamJavaFXValueViewEditForm JavaFX View/Edit Form implementation
 *	for Value.
 */
public class CFBamJavaFXValueViewEditForm
extends CFBorderPane
implements ICFBamJavaFXValuePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Value";
	...
	public CFBamJavaFXValueViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamValueObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamValueObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getValueFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFBamValueObj focus = getJavaFXFocusAsValue();
		if( focus != null ) {
			ICFBamValueEditObj editObj = (ICFBamValueEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXValuePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamValueObj argFocus = (ICFBamValueObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamValueObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsValue() != null ) {
							if( null == getJavaFXFocusAsValue().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXValuePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXValuePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
