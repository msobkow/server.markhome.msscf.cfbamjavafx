// Description: Java 11 JavaFX Attribute Pane implementation for StringCol.
...
/**
 *	CFBamJavaFXStringColAttrPane JavaFX Attribute Pane implementation
 *	for StringCol.
 */
public class CFBamJavaFXStringColAttrPane
extends CFGridPane
implements ICFBamJavaFXStringColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamStringColObj cur = getJavaFXFocusAsStringCol();
				if( cur != null ) {
					ICFBamStringColEditObj editObj = (ICFBamStringColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamStringColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXStringColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamStringColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamStringColObj getEffJavaFXFocus() {
		ICFBamStringColObj eff = (ICFBamStringColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamStringColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamStringColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamStringColObj focus = getJavaFXFocusAsStringCol();
		ICFBamStringColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamStringColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamStringColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamStringColObj)editObj.create();
