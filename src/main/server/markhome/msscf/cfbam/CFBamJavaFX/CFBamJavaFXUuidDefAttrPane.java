// Description: Java 11 JavaFX Attribute Pane implementation for UuidDef.
...
/**
 *	CFBamJavaFXUuidDefAttrPane JavaFX Attribute Pane implementation
 *	for UuidDef.
 */
public class CFBamJavaFXUuidDefAttrPane
extends CFGridPane
implements ICFBamJavaFXUuidDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuidDefObj cur = getJavaFXFocusAsUuidDef();
				if( cur != null ) {
					ICFBamUuidDefEditObj editObj = (ICFBamUuidDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuidDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuidDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamUuidDefObj getEffJavaFXFocus() {
		ICFBamUuidDefObj eff = (ICFBamUuidDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuidDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuidDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUuidDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUuidDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuidDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuidDefObj)editObj.create();
