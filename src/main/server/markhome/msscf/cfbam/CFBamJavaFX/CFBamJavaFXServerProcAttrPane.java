// Description: Java 11 JavaFX Attribute Pane implementation for ServerProc.
...
/**
 *	CFBamJavaFXServerProcAttrPane JavaFX Attribute Pane implementation
 *	for ServerProc.
 */
public class CFBamJavaFXServerProcAttrPane
extends CFGridPane
implements ICFBamJavaFXServerProcPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamServerProcObj cur = getJavaFXFocusAsServerProc();
				if( cur != null ) {
					ICFBamServerProcEditObj editObj = (ICFBamServerProcEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamServerProcObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXServerProcAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamServerProcObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamServerProcObj getEffJavaFXFocus() {
		ICFBamServerProcObj eff = (ICFBamServerProcObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamServerProcObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamServerProcObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamServerProcEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamServerProcEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamServerProcEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamServerProcObj)editObj.create();
