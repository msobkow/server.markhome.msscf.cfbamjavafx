// Description: Java 11 JavaFX Attribute Pane implementation for NmTokenType.
...
/**
 *	CFBamJavaFXNmTokenTypeAttrPane JavaFX Attribute Pane implementation
 *	for NmTokenType.
 */
public class CFBamJavaFXNmTokenTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokenTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokenTypeObj cur = getJavaFXFocusAsNmTokenType();
				if( cur != null ) {
					ICFBamNmTokenTypeEditObj editObj = (ICFBamNmTokenTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokenTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNmTokenTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokenTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamNmTokenTypeObj getEffJavaFXFocus() {
		ICFBamNmTokenTypeObj eff = (ICFBamNmTokenTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokenTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokenTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNmTokenTypeObj focus = getJavaFXFocusAsNmTokenType();
		ICFBamNmTokenTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokenTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokenTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokenTypeObj)editObj.create();
