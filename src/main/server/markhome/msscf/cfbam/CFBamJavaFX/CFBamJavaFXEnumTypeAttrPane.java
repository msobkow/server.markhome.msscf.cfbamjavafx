// Description: Java 11 JavaFX Attribute Pane implementation for EnumType.
...
/**
 *	CFBamJavaFXEnumTypeAttrPane JavaFX Attribute Pane implementation
 *	for EnumType.
 */
public class CFBamJavaFXEnumTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXEnumTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamEnumTypeObj cur = getJavaFXFocusAsEnumType();
				if( cur != null ) {
					ICFBamEnumTypeEditObj editObj = (ICFBamEnumTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamEnumTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXEnumTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamEnumTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamEnumTypeObj getEffJavaFXFocus() {
		ICFBamEnumTypeObj eff = (ICFBamEnumTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamEnumTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamEnumTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamEnumTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamEnumTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamEnumTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamEnumTypeObj)editObj.create();
