// Description: Java 11 JavaFX View/Edit Form implementation for IndexCol.
...
/**
 *	CFBamJavaFXIndexColViewEditForm JavaFX View/Edit Form implementation
 *	for IndexCol.
 */
public class CFBamJavaFXIndexColViewEditForm
extends CFBorderPane
implements ICFBamJavaFXIndexColPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit IndexCol";
	...
	public CFBamJavaFXIndexColViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamIndexColObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamIndexColObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getIndexColFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXIndexColPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamIndexColObj argFocus = (ICFBamIndexColObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamIndexColObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsIndexCol() != null ) {
							if( null == getJavaFXFocusAsIndexCol().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsIndexCol();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXIndexColPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXIndexColPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
