// Description: Java 11 JavaFX Attribute Pane implementation for UInt32Def.
...
/**
 *	CFBamJavaFXUInt32DefAttrPane JavaFX Attribute Pane implementation
 *	for UInt32Def.
 */
public class CFBamJavaFXUInt32DefAttrPane
extends CFGridPane
implements ICFBamJavaFXUInt32DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUInt32DefObj cur = getJavaFXFocusAsUInt32Def();
				if( cur != null ) {
					ICFBamUInt32DefEditObj editObj = (ICFBamUInt32DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUInt32DefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUInt32DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUInt32DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamUInt32DefObj getEffJavaFXFocus() {
		ICFBamUInt32DefObj eff = (ICFBamUInt32DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUInt32DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUInt32DefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamUInt32DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUInt32DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUInt32DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUInt32DefObj)editObj.create();
