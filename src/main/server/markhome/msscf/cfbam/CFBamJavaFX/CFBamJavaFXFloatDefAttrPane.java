// Description: Java 11 JavaFX Attribute Pane implementation for FloatDef.
...
/**
 *	CFBamJavaFXFloatDefAttrPane JavaFX Attribute Pane implementation
 *	for FloatDef.
 */
public class CFBamJavaFXFloatDefAttrPane
extends CFGridPane
implements ICFBamJavaFXFloatDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamFloatDefObj cur = getJavaFXFocusAsFloatDef();
				if( cur != null ) {
					ICFBamFloatDefEditObj editObj = (ICFBamFloatDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamFloatDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXFloatDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamFloatDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamFloatDefObj getEffJavaFXFocus() {
		ICFBamFloatDefObj eff = (ICFBamFloatDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamFloatDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamFloatDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamFloatDefObj focus = getJavaFXFocusAsFloatDef();
		ICFBamFloatDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamFloatDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamFloatDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamFloatDefObj)editObj.create();
