// Description: Java 11 JavaFX Attribute Pane implementation for TZDateType.
...
/**
 *	CFBamJavaFXTZDateTypeAttrPane JavaFX Attribute Pane implementation
 *	for TZDateType.
 */
public class CFBamJavaFXTZDateTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTZDateTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZDateTypeObj cur = getJavaFXFocusAsTZDateType();
				if( cur != null ) {
					ICFBamTZDateTypeEditObj editObj = (ICFBamTZDateTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZDateTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZDateTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZDateTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTZDateType( argFocus );
	...
	public ICFBamTZDateTypeObj getEffJavaFXFocus() {
		ICFBamTZDateTypeObj eff = (ICFBamTZDateTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZDateTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZDateTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZDateTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZDateTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZDateTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZDateTypeObj)editObj.create();
