// Description: Java 11 JavaFX Attribute Pane implementation for NmTokensType.
...
/**
 *	CFBamJavaFXNmTokensTypeAttrPane JavaFX Attribute Pane implementation
 *	for NmTokensType.
 */
public class CFBamJavaFXNmTokensTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokensTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokensTypeObj cur = getJavaFXFocusAsNmTokensType();
				if( cur != null ) {
					ICFBamNmTokensTypeEditObj editObj = (ICFBamNmTokensTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokensTypeObj focus = getEffJavaFXFocus();
		...
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamNmTokensDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamNmTokensDefBuff.MAXLEN_MAX_VALUE );
	...
	public CFBamJavaFXNmTokensTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokensTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsNmTokensType( argFocus );
	...
	public ICFBamNmTokensTypeObj getEffJavaFXFocus() {
		ICFBamNmTokensTypeObj eff = (ICFBamNmTokensTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokensTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokensTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setNmTokensValue( null );
		}
		else {
			getJavaFXEditorInitValue().setNmTokensValue( popObj.getOptionalInitValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamNmTokensTypeObj focus = getJavaFXFocusAsNmTokensType();
		ICFBamNmTokensTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokensTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		if( ( getJavaFXEditorInitValue().getNmTokensValue() != null ) && ( getJavaFXEditorInitValue().getNmTokensValue().length() <= 0 ) ) {
			editObj.setOptionalInitValue( null );
		}
		else {
			editObj.setOptionalInitValue( getJavaFXEditorInitValue().getNmTokensValue() );
		}
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokensTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokensTypeObj)editObj.create();
