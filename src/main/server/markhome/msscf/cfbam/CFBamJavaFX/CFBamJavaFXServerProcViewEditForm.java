// Description: Java 11 JavaFX View/Edit Form implementation for ServerProc.
...
/**
 *	CFBamJavaFXServerProcViewEditForm JavaFX View/Edit Form implementation
 *	for ServerProc.
 */
public class CFBamJavaFXServerProcViewEditForm
extends CFBorderPane
implements ICFBamJavaFXServerProcPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit ServerProc";
	...
	public CFBamJavaFXServerProcViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamServerProcObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamServerProcObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getServerProcFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFBamServerProcObj focus = getJavaFXFocusAsServerProc();
		if( focus != null ) {
			ICFBamServerProcEditObj editObj = (ICFBamServerProcEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXServerProcPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamServerProcObj argFocus = (ICFBamServerProcObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamServerProcObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsServerProc() != null ) {
							if( null == getJavaFXFocusAsServerProc().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsServerProc();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXServerProcPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXServerProcPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
