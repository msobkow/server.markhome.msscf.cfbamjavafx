// Description: Java 11 JavaFX Attribute Pane implementation for TZTimeType.
...
/**
 *	CFBamJavaFXTZTimeTypeAttrPane JavaFX Attribute Pane implementation
 *	for TZTimeType.
 */
public class CFBamJavaFXTZTimeTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimeTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimeTypeObj cur = getJavaFXFocusAsTZTimeType();
				if( cur != null ) {
					ICFBamTZTimeTypeEditObj editObj = (ICFBamTZTimeTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimeTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimeTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimeTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZTimeTypeObj getEffJavaFXFocus() {
		ICFBamTZTimeTypeObj eff = (ICFBamTZTimeTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimeTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimeTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimeTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimeTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZTimeTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimeTypeObj)editObj.create();
