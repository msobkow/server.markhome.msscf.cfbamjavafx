// Description: Java 11 JavaFX Attribute Pane implementation for Int64Col.
...
/**
 *	CFBamJavaFXInt64ColAttrPane JavaFX Attribute Pane implementation
 *	for Int64Col.
 */
public class CFBamJavaFXInt64ColAttrPane
extends CFGridPane
implements ICFBamJavaFXInt64ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamInt64ColObj cur = getJavaFXFocusAsInt64Col();
				if( cur != null ) {
					ICFBamInt64ColEditObj editObj = (ICFBamInt64ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamInt64ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXInt64ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamInt64ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamInt64ColObj getEffJavaFXFocus() {
		ICFBamInt64ColObj eff = (ICFBamInt64ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamInt64ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamInt64ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamInt64ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamInt64ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamInt64ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamInt64ColObj)editObj.create();
