// Description: Java 11 JavaFX List of Obj Pane implementation for BoolCol.
...
/**
 *	CFBamJavaFXBoolColListPane JavaFX List of Obj Pane implementation
 *	for BoolCol.
 */
public class CFBamJavaFXBoolColListPane
extends CFBorderPane
implements ICFBamJavaFXBoolColPaneList
{
	public static String S_FormName = "List BoolCol";
	...
	public CFBamJavaFXBoolColListPane( ICFFormManager formManager,
	...
	public CFHBox getPanelHBoxMenu() {
		if( hboxMenu == null ) {
			...
			hboxMenu.getChildren().add( buttonAddBoolCol );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							...
							if( "a853".equals( classCode ) ) {
								CFBorderPane frame = javafxSchema.getBoolColFactory().newViewEditForm( cfFormManager, selectedInstance, getViewEditClosedCallback(), false );
				...
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						if( selectedInstance != null ) {
							String classCode = selectedInstance.getClassCode();
							if( "a853".equals( classCode ) ) {
								CFBorderPane frame = javafxSchema.getBoolColFactory().newAskDeleteForm( cfFormManager, selectedInstance, getDeleteCallback() );
								((ICFBamJavaFXBoolColPaneCommon)frame).setPaneMode( CFPane.PaneMode.View );
								cfFormManager.pushForm( frame );
							}
							else {
	...
	public void adjustListButtons() {
		...
		if( buttonAddBoolCol != null ) {
			buttonAddBoolCol.setDisable( ! allowAdds );
		}
