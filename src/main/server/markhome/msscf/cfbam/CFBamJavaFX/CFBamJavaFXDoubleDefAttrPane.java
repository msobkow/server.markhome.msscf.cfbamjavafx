// Description: Java 11 JavaFX Attribute Pane implementation for DoubleDef.
...
/**
 *	CFBamJavaFXDoubleDefAttrPane JavaFX Attribute Pane implementation
 *	for DoubleDef.
 */
public class CFBamJavaFXDoubleDefAttrPane
extends CFGridPane
implements ICFBamJavaFXDoubleDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDoubleDefObj cur = getJavaFXFocusAsDoubleDef();
				if( cur != null ) {
					ICFBamDoubleDefEditObj editObj = (ICFBamDoubleDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDoubleDefObj focus = getEffJavaFXFocus();
		...
		extends CFDoubleEditor
	{
		public InitValueEditor() {
			super();
			setMinValue( CFBamDoubleDefBuff.INITVALUE_MIN_VALUE );
			setMaxValue( CFBamDoubleDefBuff.INITVALUE_MAX_VALUE );
		...
		extends CFDoubleEditor
	{
		public MinValueEditor() {
			super();
			setMinValue( CFBamDoubleDefBuff.MINVALUE_MIN_VALUE );
			setMaxValue( CFBamDoubleDefBuff.MINVALUE_MAX_VALUE );
		...
		extends CFDoubleEditor
	{
		public MaxValueEditor() {
			super();
			setMinValue( CFBamDoubleDefBuff.MAXVALUE_MIN_VALUE );
			setMaxValue( CFBamDoubleDefBuff.MAXVALUE_MAX_VALUE );
	...
	public CFBamJavaFXDoubleDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDoubleDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDoubleDef( argFocus );
	...
	public ICFBamDoubleDefObj getEffJavaFXFocus() {
		ICFBamDoubleDefObj eff = (ICFBamDoubleDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDoubleDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDoubleDefObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setDoubleValue( null );
		}
		else {
			getJavaFXEditorInitValue().setDoubleValue( popObj.getOptionalInitValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMinValue().setDoubleValue( null );
		}
		else {
			getJavaFXEditorMinValue().setDoubleValue( popObj.getOptionalMinValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMaxValue().setDoubleValue( null );
		}
		else {
			getJavaFXEditorMaxValue().setDoubleValue( popObj.getOptionalMaxValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamDoubleDefObj focus = getJavaFXFocusAsDoubleDef();
		ICFBamDoubleDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDoubleDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getDoubleValue() );

		editObj.setOptionalMinValue( getJavaFXEditorMinValue().getDoubleValue() );

		editObj.setOptionalMaxValue( getJavaFXEditorMaxValue().getDoubleValue() );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDoubleDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDoubleDefObj)editObj.create();
