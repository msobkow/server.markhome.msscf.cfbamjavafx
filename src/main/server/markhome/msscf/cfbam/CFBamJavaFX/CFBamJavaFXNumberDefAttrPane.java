// Description: Java 11 JavaFX Attribute Pane implementation for NumberDef.
...
/**
 *	CFBamJavaFXNumberDefAttrPane JavaFX Attribute Pane implementation
 *	for NumberDef.
 */
public class CFBamJavaFXNumberDefAttrPane
extends CFGridPane
implements ICFBamJavaFXNumberDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNumberDefObj cur = getJavaFXFocusAsNumberDef();
				if( cur != null ) {
					ICFBamNumberDefEditObj editObj = (ICFBamNumberDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNumberDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNumberDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNumberDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamNumberDefObj getEffJavaFXFocus() {
		ICFBamNumberDefObj eff = (ICFBamNumberDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNumberDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNumberDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNumberDefObj focus = getJavaFXFocusAsNumberDef();
		ICFBamNumberDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNumberDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNumberDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNumberDefObj)editObj.create();
