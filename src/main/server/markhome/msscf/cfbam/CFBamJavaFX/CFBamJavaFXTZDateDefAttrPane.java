// Description: Java 11 JavaFX Attribute Pane implementation for TZDateDef.
...
/**
 *	CFBamJavaFXTZDateDefAttrPane JavaFX Attribute Pane implementation
 *	for TZDateDef.
 */
public class CFBamJavaFXTZDateDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTZDateDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZDateDefObj cur = getJavaFXFocusAsTZDateDef();
				if( cur != null ) {
					ICFBamTZDateDefEditObj editObj = (ICFBamTZDateDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZDateDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZDateDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZDateDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZDateDefObj getEffJavaFXFocus() {
		ICFBamTZDateDefObj eff = (ICFBamTZDateDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZDateDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZDateDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZDateDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZDateDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZDateDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZDateDefObj)editObj.create();
