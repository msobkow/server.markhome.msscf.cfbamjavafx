// Description: Java 11 JavaFX Attribute Pane implementation for NmTokensCol.
...
/**
 *	CFBamJavaFXNmTokensColAttrPane JavaFX Attribute Pane implementation
 *	for NmTokensCol.
 */
public class CFBamJavaFXNmTokensColAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokensColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokensColObj cur = getJavaFXFocusAsNmTokensCol();
				if( cur != null ) {
					ICFBamNmTokensColEditObj editObj = (ICFBamNmTokensColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokensColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNmTokensColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokensColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsNmTokensCol( argFocus );
	...
	public ICFBamNmTokensColObj getEffJavaFXFocus() {
		ICFBamNmTokensColObj eff = (ICFBamNmTokensColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokensColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokensColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNmTokensColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokensColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokensColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokensColObj)editObj.create();
