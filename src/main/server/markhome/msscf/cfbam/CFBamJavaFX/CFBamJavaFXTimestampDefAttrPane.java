// Description: Java 11 JavaFX Attribute Pane implementation for TimestampDef.
...
/**
 *	CFBamJavaFXTimestampDefAttrPane JavaFX Attribute Pane implementation
 *	for TimestampDef.
 */
public class CFBamJavaFXTimestampDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTimestampDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTimestampDefObj cur = getJavaFXFocusAsTimestampDef();
				if( cur != null ) {
					ICFBamTimestampDefEditObj editObj = (ICFBamTimestampDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTimestampDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTimestampDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimestampDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTimestampDefObj getEffJavaFXFocus() {
		ICFBamTimestampDefObj eff = (ICFBamTimestampDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTimestampDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTimestampDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTimestampDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTimestampDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTimestampDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTimestampDefObj)editObj.create();
