// Description: Java 11 JavaFX Attribute Pane implementation for BlobType.
...
/**
 *	CFBamJavaFXBlobTypeAttrPane JavaFX Attribute Pane implementation
 *	for BlobType.
 */
public class CFBamJavaFXBlobTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXBlobTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamBlobTypeObj cur = getJavaFXFocusAsBlobType();
				if( cur != null ) {
					ICFBamBlobTypeEditObj editObj = (ICFBamBlobTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamBlobTypeObj focus = getEffJavaFXFocus();
		...
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamBlobDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamBlobDefBuff.MAXLEN_MAX_VALUE );
	...
	public CFBamJavaFXBlobTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBlobTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamBlobTypeObj getEffJavaFXFocus() {
		ICFBamBlobTypeObj eff = (ICFBamBlobTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamBlobTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamBlobTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamBlobTypeObj focus = getJavaFXFocusAsBlobType();
		ICFBamBlobTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamBlobTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamBlobTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamBlobTypeObj)editObj.create();
