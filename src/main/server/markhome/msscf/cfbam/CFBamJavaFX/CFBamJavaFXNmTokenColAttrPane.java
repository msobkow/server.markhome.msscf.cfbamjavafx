// Description: Java 11 JavaFX Attribute Pane implementation for NmTokenCol.
...
/**
 *	CFBamJavaFXNmTokenColAttrPane JavaFX Attribute Pane implementation
 *	for NmTokenCol.
 */
public class CFBamJavaFXNmTokenColAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokenColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokenColObj cur = getJavaFXFocusAsNmTokenCol();
				if( cur != null ) {
					ICFBamNmTokenColEditObj editObj = (ICFBamNmTokenColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokenColObj focus = getEffJavaFXFocus();
		...
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamNmTokenDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamNmTokenDefBuff.MAXLEN_MAX_VALUE );
	...
	public CFBamJavaFXNmTokenColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokenColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsNmTokenCol( argFocus );
	...
	public ICFBamNmTokenColObj getEffJavaFXFocus() {
		ICFBamNmTokenColObj eff = (ICFBamNmTokenColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokenColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokenColObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setNmTokenValue( null );
		}
		else {
			getJavaFXEditorInitValue().setNmTokenValue( popObj.getOptionalInitValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamNmTokenColObj focus = getJavaFXFocusAsNmTokenCol();
		ICFBamNmTokenColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokenColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		if( ( getJavaFXEditorInitValue().getNmTokenValue() != null ) && ( getJavaFXEditorInitValue().getNmTokenValue().length() <= 0 ) ) {
			editObj.setOptionalInitValue( null );
		}
		else {
			editObj.setOptionalInitValue( getJavaFXEditorInitValue().getNmTokenValue() );
		}
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokenColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokenColObj)editObj.create();
