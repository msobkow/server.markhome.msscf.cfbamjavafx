// Description: Java 11 JavaFX View/Edit Form implementation for Atom.
...
/**
 *	CFBamJavaFXAtomViewEditForm JavaFX View/Edit Form implementation
 *	for Atom.
 */
public class CFBamJavaFXAtomViewEditForm
extends CFBorderPane
implements ICFBamJavaFXAtomPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Atom";
	...
	public CFBamJavaFXAtomViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamAtomObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamAtomObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getAtomFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFBamAtomObj focus = getJavaFXFocusAsAtom();
		if( focus != null ) {
			ICFBamAtomEditObj editObj = (ICFBamAtomEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXAtomPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamAtomObj argFocus = (ICFBamAtomObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamAtomObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsAtom() != null ) {
							if( null == getJavaFXFocusAsAtom().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXAtomPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXAtomPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
