// Description: Java 11 JavaFX Attribute Pane implementation for PopTopDep.
...
/**
 *	CFBamJavaFXPopTopDepAttrPane JavaFX Attribute Pane implementation
 *	for PopTopDep.
 */
public class CFBamJavaFXPopTopDepAttrPane
extends CFGridPane
implements ICFBamJavaFXPopTopDepPaneCommon
{
	...
	implements ICFBamJavaFXRelationChosen
	{
		...
		public void choseRelation( ICFBamRelationObj value ) {
			if( javafxReferenceLookupRelation != null ) {
				ICFBamPopTopDepObj cur = getJavaFXFocusAsPopTopDep();
				if( cur != null ) {
					ICFBamPopTopDepEditObj editObj = (ICFBamPopTopDepEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamPopTopDepObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXPopTopDepAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopTopDepObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamPopTopDepObj getEffJavaFXFocus() {
		ICFBamPopTopDepObj eff = (ICFBamPopTopDepObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamPopTopDepObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamPopTopDepObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamPopTopDepEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamPopTopDepEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamPopTopDepEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamPopTopDepObj)editObj.create();
