// Description: Java 11 JavaFX Attribute Pane implementation for ClearDep.
...
/**
 *	CFBamJavaFXClearDepAttrPane JavaFX Attribute Pane implementation
 *	for ClearDep.
 */
public class CFBamJavaFXClearDepAttrPane
extends CFGridPane
implements ICFBamJavaFXClearDepPaneCommon
{
	...
	implements ICFBamJavaFXRelationChosen
	{
		...
		public void choseRelation( ICFBamRelationObj value ) {
			if( javafxReferenceLookupRelation != null ) {
				ICFBamClearDepObj cur = getJavaFXFocusAsClearDep();
				if( cur != null ) {
					ICFBamClearDepEditObj editObj = (ICFBamClearDepEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamClearDepObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXClearDepAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamClearDepObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamClearDepObj getEffJavaFXFocus() {
		ICFBamClearDepObj eff = (ICFBamClearDepObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamClearDepObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamClearDepObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamClearDepEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamClearDepEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamClearDepEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamClearDepObj)editObj.create();
