// Description: Java 11 JavaFX Attribute Pane implementation for ServerListFunc.
...
/**
 *	CFBamJavaFXServerListFuncAttrPane JavaFX Attribute Pane implementation
 *	for ServerListFunc.
 */
public class CFBamJavaFXServerListFuncAttrPane
extends CFGridPane
implements ICFBamJavaFXServerListFuncPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamServerListFuncObj cur = getJavaFXFocusAsServerListFunc();
				if( cur != null ) {
					ICFBamServerListFuncEditObj editObj = (ICFBamServerListFuncEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamServerListFuncObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXServerListFuncAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamServerListFuncObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamServerListFuncObj getEffJavaFXFocus() {
		ICFBamServerListFuncObj eff = (ICFBamServerListFuncObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamServerListFuncObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamServerListFuncObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamServerListFuncEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamServerListFuncEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamServerListFuncEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamServerListFuncObj)editObj.create();
