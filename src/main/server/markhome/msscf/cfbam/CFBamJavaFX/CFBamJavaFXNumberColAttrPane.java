// Description: Java 11 JavaFX Attribute Pane implementation for NumberCol.
...
/**
 *	CFBamJavaFXNumberColAttrPane JavaFX Attribute Pane implementation
 *	for NumberCol.
 */
public class CFBamJavaFXNumberColAttrPane
extends CFGridPane
implements ICFBamJavaFXNumberColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNumberColObj cur = getJavaFXFocusAsNumberCol();
				if( cur != null ) {
					ICFBamNumberColEditObj editObj = (ICFBamNumberColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNumberColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNumberColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNumberColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamNumberColObj getEffJavaFXFocus() {
		ICFBamNumberColObj eff = (ICFBamNumberColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNumberColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNumberColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNumberColObj focus = getJavaFXFocusAsNumberCol();
		ICFBamNumberColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNumberColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNumberColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNumberColObj)editObj.create();
