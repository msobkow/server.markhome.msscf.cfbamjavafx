// Description: Java 11 JavaFX View/Edit Form implementation for TableCol.
...
/**
 *	CFBamJavaFXTableColViewEditForm JavaFX View/Edit Form implementation
 *	for TableCol.
 */
public class CFBamJavaFXTableColViewEditForm
extends CFBorderPane
implements ICFBamJavaFXTableColPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit TableCol";
	...
	public CFBamJavaFXTableColViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTableColObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamTableColObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getTableColFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamTableColEditObj editObj = (ICFBamTableColEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXTableColPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamTableColObj argFocus = (ICFBamTableColObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamTableColObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsTableCol() != null ) {
							if( null == getJavaFXFocusAsTableCol().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsTableCol();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXTableColPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXTableColPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
