// Description: Java 11 JavaFX Attribute Pane implementation for NmTokensDef.
...
/**
 *	CFBamJavaFXNmTokensDefAttrPane JavaFX Attribute Pane implementation
 *	for NmTokensDef.
 */
public class CFBamJavaFXNmTokensDefAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokensDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokensDefObj cur = getJavaFXFocusAsNmTokensDef();
				if( cur != null ) {
					ICFBamNmTokensDefEditObj editObj = (ICFBamNmTokensDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokensDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNmTokensDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokensDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsNmTokensDef( argFocus );
	...
	public ICFBamNmTokensDefObj getEffJavaFXFocus() {
		ICFBamNmTokensDefObj eff = (ICFBamNmTokensDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokensDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokensDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNmTokensDefObj focus = getJavaFXFocusAsNmTokensDef();
		ICFBamNmTokensDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokensDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokensDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokensDefObj)editObj.create();
