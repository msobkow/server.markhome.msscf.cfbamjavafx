// Description: Java 11 JavaFX Attribute Pane implementation for TimeDef.
...
/**
 *	CFBamJavaFXTimeDefAttrPane JavaFX Attribute Pane implementation
 *	for TimeDef.
 */
public class CFBamJavaFXTimeDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTimeDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTimeDefObj cur = getJavaFXFocusAsTimeDef();
				if( cur != null ) {
					ICFBamTimeDefEditObj editObj = (ICFBamTimeDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTimeDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTimeDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimeDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTimeDefObj getEffJavaFXFocus() {
		ICFBamTimeDefObj eff = (ICFBamTimeDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTimeDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTimeDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTimeDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTimeDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTimeDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTimeDefObj)editObj.create();
