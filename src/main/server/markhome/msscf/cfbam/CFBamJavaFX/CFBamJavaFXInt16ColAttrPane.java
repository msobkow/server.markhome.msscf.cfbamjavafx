// Description: Java 11 JavaFX Attribute Pane implementation for Int16Col.
...
/**
 *	CFBamJavaFXInt16ColAttrPane JavaFX Attribute Pane implementation
 *	for Int16Col.
 */
public class CFBamJavaFXInt16ColAttrPane
extends CFGridPane
implements ICFBamJavaFXInt16ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamInt16ColObj cur = getJavaFXFocusAsInt16Col();
				if( cur != null ) {
					ICFBamInt16ColEditObj editObj = (ICFBamInt16ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamInt16ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXInt16ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamInt16ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamInt16ColObj getEffJavaFXFocus() {
		ICFBamInt16ColObj eff = (ICFBamInt16ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamInt16ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamInt16ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamInt16ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamInt16ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamInt16ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamInt16ColObj)editObj.create();
