// Description: Java 11 JavaFX Attribute Pane implementation for TextCol.
...
/**
 *	CFBamJavaFXTextColAttrPane JavaFX Attribute Pane implementation
 *	for TextCol.
 */
public class CFBamJavaFXTextColAttrPane
extends CFGridPane
implements ICFBamJavaFXTextColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTextColObj cur = getJavaFXFocusAsTextCol();
				if( cur != null ) {
					ICFBamTextColEditObj editObj = (ICFBamTextColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTextColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTextColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTextColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTextColObj getEffJavaFXFocus() {
		ICFBamTextColObj eff = (ICFBamTextColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTextColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTextColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTextColObj focus = getJavaFXFocusAsTextCol();
		ICFBamTextColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTextColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTextColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTextColObj)editObj.create();
