// Description: Java 11 JavaFX Attribute Pane implementation for Int32Col.
...
/**
 *	CFBamJavaFXInt32ColAttrPane JavaFX Attribute Pane implementation
 *	for Int32Col.
 */
public class CFBamJavaFXInt32ColAttrPane
extends CFGridPane
implements ICFBamJavaFXInt32ColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamInt32ColObj cur = getJavaFXFocusAsInt32Col();
				if( cur != null ) {
					ICFBamInt32ColEditObj editObj = (ICFBamInt32ColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamInt32ColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXInt32ColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamInt32ColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamInt32ColObj getEffJavaFXFocus() {
		ICFBamInt32ColObj eff = (ICFBamInt32ColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamInt32ColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamInt32ColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamInt32ColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamInt32ColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamInt32ColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamInt32ColObj)editObj.create();
