// Description: Java 11 JavaFX View/Edit Form implementation for DoubleType.
...
/**
 *	CFBamJavaFXDoubleTypeViewEditForm JavaFX View/Edit Form implementation
 *	for DoubleType.
 */
public class CFBamJavaFXDoubleTypeViewEditForm
extends CFBorderPane
implements ICFBamJavaFXDoubleTypePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit DoubleType";
	...
	public CFBamJavaFXDoubleTypeViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDoubleTypeObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamDoubleTypeObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getDoubleTypeFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamDoubleTypeEditObj editObj = (ICFBamDoubleTypeEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXDoubleTypePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamDoubleTypeObj argFocus = (ICFBamDoubleTypeObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamDoubleTypeObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsDoubleType() != null ) {
							if( null == getJavaFXFocusAsDoubleType().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsDoubleType();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXDoubleTypePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXDoubleTypePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
