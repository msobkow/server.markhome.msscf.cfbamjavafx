// Description: Java 11 JavaFX Attribute Pane implementation for FloatType.
...
/**
 *	CFBamJavaFXFloatTypeAttrPane JavaFX Attribute Pane implementation
 *	for FloatType.
 */
public class CFBamJavaFXFloatTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXFloatTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamFloatTypeObj cur = getJavaFXFocusAsFloatType();
				if( cur != null ) {
					ICFBamFloatTypeEditObj editObj = (ICFBamFloatTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamFloatTypeObj focus = getEffJavaFXFocus();
		...
		extends CFFloatEditor
	{
		public InitValueEditor() {
			super();
			setMinValue( CFBamFloatDefBuff.INITVALUE_MIN_VALUE );
			setMaxValue( CFBamFloatDefBuff.INITVALUE_MAX_VALUE );
		...
		extends CFFloatEditor
	{
		public MinValueEditor() {
			super();
			setMinValue( CFBamFloatDefBuff.MINVALUE_MIN_VALUE );
			setMaxValue( CFBamFloatDefBuff.MINVALUE_MAX_VALUE );
		...
		extends CFFloatEditor
	{
		public MaxValueEditor() {
			super();
			setMinValue( CFBamFloatDefBuff.MAXVALUE_MIN_VALUE );
			setMaxValue( CFBamFloatDefBuff.MAXVALUE_MAX_VALUE );
	...
	public CFBamJavaFXFloatTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamFloatTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamFloatTypeObj getEffJavaFXFocus() {
		ICFBamFloatTypeObj eff = (ICFBamFloatTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamFloatTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamFloatTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setFloatValue( null );
		}
		else {
			getJavaFXEditorInitValue().setFloatValue( popObj.getOptionalInitValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMinValue().setFloatValue( null );
		}
		else {
			getJavaFXEditorMinValue().setFloatValue( popObj.getOptionalMinValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMaxValue().setFloatValue( null );
		}
		else {
			getJavaFXEditorMaxValue().setFloatValue( popObj.getOptionalMaxValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamFloatTypeObj focus = getJavaFXFocusAsFloatType();
		ICFBamFloatTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamFloatTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getFloatValue() );

		editObj.setOptionalMinValue( getJavaFXEditorMinValue().getFloatValue() );

		editObj.setOptionalMaxValue( getJavaFXEditorMaxValue().getFloatValue() );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamFloatTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamFloatTypeObj)editObj.create();
