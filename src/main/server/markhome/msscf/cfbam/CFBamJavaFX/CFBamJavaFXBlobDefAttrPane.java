// Description: Java 11 JavaFX Attribute Pane implementation for BlobDef.
...
/**
 *	CFBamJavaFXBlobDefAttrPane JavaFX Attribute Pane implementation
 *	for BlobDef.
 */
public class CFBamJavaFXBlobDefAttrPane
extends CFGridPane
implements ICFBamJavaFXBlobDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamBlobDefObj cur = getJavaFXFocusAsBlobDef();
				if( cur != null ) {
					ICFBamBlobDefEditObj editObj = (ICFBamBlobDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamBlobDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXBlobDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBlobDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamBlobDefObj getEffJavaFXFocus() {
		ICFBamBlobDefObj eff = (ICFBamBlobDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamBlobDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamBlobDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamBlobDefObj focus = getJavaFXFocusAsBlobDef();
		ICFBamBlobDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamBlobDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamBlobDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamBlobDefObj)editObj.create();
