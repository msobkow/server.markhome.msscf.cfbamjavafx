// Description: Java 11 JavaFX Attribute Pane implementation for PopSubDep2.
...
/**
 *	CFBamJavaFXPopSubDep2AttrPane JavaFX Attribute Pane implementation
 *	for PopSubDep2.
 */
public class CFBamJavaFXPopSubDep2AttrPane
extends CFGridPane
implements ICFBamJavaFXPopSubDep2PaneCommon
{
	...
	implements ICFBamJavaFXRelationChosen
	{
		...
		public void choseRelation( ICFBamRelationObj value ) {
			if( javafxReferenceLookupRelation != null ) {
				ICFBamPopSubDep2Obj cur = getJavaFXFocusAsPopSubDep2();
				if( cur != null ) {
					ICFBamPopSubDep2EditObj editObj = (ICFBamPopSubDep2EditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamPopSubDep2Obj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXPopSubDep2AttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopSubDep2Obj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamPopSubDep2Obj getEffJavaFXFocus() {
		ICFBamPopSubDep2Obj eff = (ICFBamPopSubDep2Obj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamPopSubDep2Obj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamPopSubDep2Obj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamPopSubDep2EditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamPopSubDep2EditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamPopSubDep2EditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamPopSubDep2Obj)editObj.create();
