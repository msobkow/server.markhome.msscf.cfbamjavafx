// Description: Java 11 JavaFX Attribute Pane implementation for StringDef.
...
/**
 *	CFBamJavaFXStringDefAttrPane JavaFX Attribute Pane implementation
 *	for StringDef.
 */
public class CFBamJavaFXStringDefAttrPane
extends CFGridPane
implements ICFBamJavaFXStringDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamStringDefObj cur = getJavaFXFocusAsStringDef();
				if( cur != null ) {
					ICFBamStringDefEditObj editObj = (ICFBamStringDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamStringDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXStringDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamStringDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamStringDefObj getEffJavaFXFocus() {
		ICFBamStringDefObj eff = (ICFBamStringDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamStringDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamStringDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamStringDefObj focus = getJavaFXFocusAsStringDef();
		ICFBamStringDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamStringDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamStringDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamStringDefObj)editObj.create();
