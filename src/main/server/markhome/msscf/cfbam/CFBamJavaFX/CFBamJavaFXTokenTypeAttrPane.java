// Description: Java 11 JavaFX Attribute Pane implementation for TokenType.
...
/**
 *	CFBamJavaFXTokenTypeAttrPane JavaFX Attribute Pane implementation
 *	for TokenType.
 */
public class CFBamJavaFXTokenTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTokenTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTokenTypeObj cur = getJavaFXFocusAsTokenType();
				if( cur != null ) {
					ICFBamTokenTypeEditObj editObj = (ICFBamTokenTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTokenTypeObj focus = getEffJavaFXFocus();
		...
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamTokenDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamTokenDefBuff.MAXLEN_MAX_VALUE );
	...
	public CFBamJavaFXTokenTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTokenTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTokenTypeObj getEffJavaFXFocus() {
		ICFBamTokenTypeObj eff = (ICFBamTokenTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTokenTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTokenTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setTokenValue( null );
		}
		else {
			getJavaFXEditorInitValue().setTokenValue( popObj.getOptionalInitValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamTokenTypeObj focus = getJavaFXFocusAsTokenType();
		ICFBamTokenTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTokenTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		if( ( getJavaFXEditorInitValue().getTokenValue() != null ) && ( getJavaFXEditorInitValue().getTokenValue().length() <= 0 ) ) {
			editObj.setOptionalInitValue( null );
		}
		else {
			editObj.setOptionalInitValue( getJavaFXEditorInitValue().getTokenValue() );
		}
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTokenTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTokenTypeObj)editObj.create();
