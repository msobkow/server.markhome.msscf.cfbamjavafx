// Description: Java 11 JavaFX View/Edit Form implementation for PopDep.
...
/**
 *	CFBamJavaFXPopDepViewEditForm JavaFX View/Edit Form implementation
 *	for PopDep.
 */
public class CFBamJavaFXPopDepViewEditForm
extends CFBorderPane
implements ICFBamJavaFXPopDepPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit PopDep";
	...
	public CFBamJavaFXPopDepViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopDepObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamPopDepObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getPopDepFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamPopDepEditObj editObj = (ICFBamPopDepEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXPopDepPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamPopDepObj argFocus = (ICFBamPopDepObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamPopDepObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsPopDep() != null ) {
							if( null == getJavaFXFocusAsPopDep().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXPopDepPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXPopDepPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
