// Description: Java 11 JavaFX View/Edit Form implementation for UuidGen.
...
/**
 *	CFBamJavaFXUuidGenViewEditForm JavaFX View/Edit Form implementation
 *	for UuidGen.
 */
public class CFBamJavaFXUuidGenViewEditForm
extends CFBorderPane
implements ICFBamJavaFXUuidGenPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit UuidGen";
	...
	public CFBamJavaFXUuidGenViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidGenObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamUuidGenObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getUuidGenFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamUuidGenEditObj editObj = (ICFBamUuidGenEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXUuidGenPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamUuidGenObj argFocus = (ICFBamUuidGenObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamUuidGenObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsUuidGen() != null ) {
							if( null == getJavaFXFocusAsUuidGen().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXUuidGenPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXUuidGenPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
