// Description: Java 11 JavaFX View/Edit Form implementation for UuidCol.
...
/**
 *	CFBamJavaFXUuidColViewEditForm JavaFX View/Edit Form implementation
 *	for UuidCol.
 */
public class CFBamJavaFXUuidColViewEditForm
extends CFBorderPane
implements ICFBamJavaFXUuidColPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit UuidCol";
	...
	public CFBamJavaFXUuidColViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidColObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamUuidColObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getUuidColFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamUuidColEditObj editObj = (ICFBamUuidColEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXUuidColPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamUuidColObj argFocus = (ICFBamUuidColObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamUuidColObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsUuidCol() != null ) {
							if( null == getJavaFXFocusAsUuidCol().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXUuidColPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXUuidColPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
