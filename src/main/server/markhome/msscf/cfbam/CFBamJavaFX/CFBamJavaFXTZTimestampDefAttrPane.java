// Description: Java 11 JavaFX Attribute Pane implementation for TZTimestampDef.
...
/**
 *	CFBamJavaFXTZTimestampDefAttrPane JavaFX Attribute Pane implementation
 *	for TZTimestampDef.
 */
public class CFBamJavaFXTZTimestampDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimestampDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimestampDefObj cur = getJavaFXFocusAsTZTimestampDef();
				if( cur != null ) {
					ICFBamTZTimestampDefEditObj editObj = (ICFBamTZTimestampDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimestampDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimestampDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimestampDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTZTimestampDef( argFocus );
	...
	public ICFBamTZTimestampDefObj getEffJavaFXFocus() {
		ICFBamTZTimestampDefObj eff = (ICFBamTZTimestampDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimestampDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimestampDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimestampDefObj focus = getJavaFXFocusAsTZTimestampDef();
		ICFBamTZTimestampDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimestampDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZTimestampDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimestampDefObj)editObj.create();
