// Description: Java 11 JavaFX View/Edit Form implementation for StringCol.
...
/**
 *	CFBamJavaFXStringColViewEditForm JavaFX View/Edit Form implementation
 *	for StringCol.
 */
public class CFBamJavaFXStringColViewEditForm
extends CFBorderPane
implements ICFBamJavaFXStringColPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit StringCol";
	...
	public CFBamJavaFXStringColViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamStringColObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamStringColObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getStringColFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamStringColEditObj editObj = (ICFBamStringColEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXStringColPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamStringColObj argFocus = (ICFBamStringColObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamStringColObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsStringCol() != null ) {
							if( null == getJavaFXFocusAsStringCol().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsStringCol();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXStringColPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXStringColPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
