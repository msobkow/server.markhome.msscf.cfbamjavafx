// Description: Java 11 JavaFX Attribute Pane implementation for TextType.
...
/**
 *	CFBamJavaFXTextTypeAttrPane JavaFX Attribute Pane implementation
 *	for TextType.
 */
public class CFBamJavaFXTextTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTextTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTextTypeObj cur = getJavaFXFocusAsTextType();
				if( cur != null ) {
					ICFBamTextTypeEditObj editObj = (ICFBamTextTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTextTypeObj focus = getEffJavaFXFocus();
		...
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamTextDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamTextDefBuff.MAXLEN_MAX_VALUE );
	...
	public CFBamJavaFXTextTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTextTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTextTypeObj getEffJavaFXFocus() {
		ICFBamTextTypeObj eff = (ICFBamTextTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTextTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTextTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setTextValue( null );
		}
		else {
			getJavaFXEditorInitValue().setTextValue( popObj.getOptionalInitValue() );
		}
	...
	public void postFields()
	{
		...
		ICFBamTextTypeObj focus = getJavaFXFocusAsTextType();
		ICFBamTextTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTextTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		if( ( getJavaFXEditorInitValue().getTextValue() != null ) && ( getJavaFXEditorInitValue().getTextValue().length() <= 0 ) ) {
			editObj.setOptionalInitValue( null );
		}
		else {
			editObj.setOptionalInitValue( getJavaFXEditorInitValue().getTextValue() );
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTextTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTextTypeObj)editObj.create();
