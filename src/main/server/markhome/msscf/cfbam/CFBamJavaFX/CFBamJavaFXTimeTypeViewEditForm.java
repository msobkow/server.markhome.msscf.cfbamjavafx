// Description: Java 11 JavaFX View/Edit Form implementation for TimeType.
...
/**
 *	CFBamJavaFXTimeTypeViewEditForm JavaFX View/Edit Form implementation
 *	for TimeType.
 */
public class CFBamJavaFXTimeTypeViewEditForm
extends CFBorderPane
implements ICFBamJavaFXTimeTypePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit TimeType";
	...
	public CFBamJavaFXTimeTypeViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimeTypeObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamTimeTypeObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getTimeTypeFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFBamTimeTypeObj focus = getJavaFXFocusAsTimeType();
		if( focus != null ) {
			ICFBamTimeTypeEditObj editObj = (ICFBamTimeTypeEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXTimeTypePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamTimeTypeObj argFocus = (ICFBamTimeTypeObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamTimeTypeObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsTimeType() != null ) {
							if( null == getJavaFXFocusAsTimeType().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsTimeType();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXTimeTypePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXTimeTypePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
