// Description: Java 11 JavaFX View/Edit Form implementation for Relation.
...
/**
 *	CFBamJavaFXRelationViewEditForm JavaFX View/Edit Form implementation
 *	for Relation.
 */
public class CFBamJavaFXRelationViewEditForm
extends CFBorderPane
implements ICFBamJavaFXRelationPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit Relation";
	...
	public CFBamJavaFXRelationViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamRelationObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamRelationObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getRelationFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXRelationPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamRelationObj argFocus = (ICFBamRelationObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamRelationObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsRelation() != null ) {
							if( null == getJavaFXFocusAsRelation().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsRelation();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXRelationPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXRelationPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
