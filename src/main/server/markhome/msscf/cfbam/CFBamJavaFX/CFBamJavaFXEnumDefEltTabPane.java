// Description: Java 11 JavaFX Element TabPane implementation for EnumDef.
...
/**
 *	CFBamJavaFXEnumDefEltTabPane JavaFX Element TabPane implementation
 *	for EnumDef.
 */
public class CFBamJavaFXEnumDefEltTabPane
extends CFTabPane
implements ICFBamJavaFXEnumDefPaneCommon
{
	...
	public final String LABEL_TabComponentsTagList = "Required Components Component Enum Tags";
	protected CFTab tabComponentsTag = null;
	...
	protected CFBorderPane tabViewComponentsTagListPane = null;

	public CFBamJavaFXEnumDefEltTabPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamEnumDefObj argFocus ) {
		super();
		...
		tabComponentsTag = new CFTab();
		tabComponentsTag.setText( LABEL_TabComponentsTagList );
		tabComponentsTag.setContent( getTabViewComponentsTagListPane() );
		getTabs().add( tabComponentsTag );
	...
	implements ICFRefreshCallback
	{
		...
		public void refreshMe() {
			...
			ICFBamEnumDefObj focus = (ICFBamEnumDefObj)getJavaFXFocusAsEnumDef();
			if( focus != null ) {
	...
	protected class RefreshComponentsTagList
	implements ICFRefreshCallback
	{
		...
		public void refreshMe() {
			Collection<ICFBamEnumTagObj> dataCollection;
			ICFBamEnumDefObj focus = (ICFBamEnumDefObj)getJavaFXFocusAsEnumDef();
			if( focus != null ) {
				dataCollection = focus.getRequiredComponentsTag( javafxIsInitializing );
			}
			else {
				dataCollection = null;
			}
			CFBorderPane pane = getTabViewComponentsTagListPane();
			ICFBamJavaFXEnumTagPaneList jpList = (ICFBamJavaFXEnumTagPaneList)pane;
			jpList.setJavaFXDataCollection( dataCollection );
		}
	}

	public CFBorderPane getTabViewComponentsTagListPane() {
		if( tabViewComponentsTagListPane == null ) {
			ICFBamEnumDefObj focus = (ICFBamEnumDefObj)getJavaFXFocusAsEnumDef();
			Collection<ICFBamEnumTagObj> dataCollection;
			if( focus != null ) {
				dataCollection = focus.getRequiredComponentsTag( javafxIsInitializing );
			...
			ICFBamEnumDefObj javafxContainer;
			if( ( focus != null ) && ( focus instanceof ICFBamEnumDefObj ) ) {
				javafxContainer = (ICFBamEnumDefObj)focus;
			}
			else {
				javafxContainer = null;
			}
			tabViewComponentsTagListPane = javafxSchema.getEnumTagFactory().newListPane( cfFormManager, javafxContainer, null, dataCollection, new RefreshComponentsTagList(), true );
		}
		return( tabViewComponentsTagListPane );
	}
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		if( tabViewComponentsTagListPane != null ) {
			((ICFBamJavaFXEnumTagPaneCommon)tabViewComponentsTagListPane).setPaneMode( value );
		}
	}
}
