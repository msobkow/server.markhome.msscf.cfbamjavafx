// Description: Java 11 JavaFX Attribute Pane implementation for TokenCol.
...
/**
 *	CFBamJavaFXTokenColAttrPane JavaFX Attribute Pane implementation
 *	for TokenCol.
 */
public class CFBamJavaFXTokenColAttrPane
extends CFGridPane
implements ICFBamJavaFXTokenColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTokenColObj cur = getJavaFXFocusAsTokenCol();
				if( cur != null ) {
					ICFBamTokenColEditObj editObj = (ICFBamTokenColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTokenColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTokenColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTokenColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTokenColObj getEffJavaFXFocus() {
		ICFBamTokenColObj eff = (ICFBamTokenColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTokenColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTokenColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTokenColObj focus = getJavaFXFocusAsTokenCol();
		ICFBamTokenColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTokenColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTokenColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTokenColObj)editObj.create();
