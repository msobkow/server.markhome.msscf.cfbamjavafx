// Description: Java 11 JavaFX View/Edit Form implementation for PopTopDep.
...
/**
 *	CFBamJavaFXPopTopDepViewEditForm JavaFX View/Edit Form implementation
 *	for PopTopDep.
 */
public class CFBamJavaFXPopTopDepViewEditForm
extends CFBorderPane
implements ICFBamJavaFXPopTopDepPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit PopTopDep";
	...
	public CFBamJavaFXPopTopDepViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopTopDepObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamPopTopDepObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getPopTopDepFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		ICFBamPopTopDepObj focus = getJavaFXFocusAsPopTopDep();
		if( focus != null ) {
			ICFBamPopTopDepEditObj editObj = (ICFBamPopTopDepEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXPopTopDepPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamPopTopDepObj argFocus = (ICFBamPopTopDepObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamPopTopDepObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsPopTopDep() != null ) {
							if( null == getJavaFXFocusAsPopTopDep().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsPopTopDep();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXPopTopDepPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXPopTopDepPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
