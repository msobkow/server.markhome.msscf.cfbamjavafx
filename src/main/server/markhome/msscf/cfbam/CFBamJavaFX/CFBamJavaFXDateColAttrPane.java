// Description: Java 11 JavaFX Attribute Pane implementation for DateCol.
...
/**
 *	CFBamJavaFXDateColAttrPane JavaFX Attribute Pane implementation
 *	for DateCol.
 */
public class CFBamJavaFXDateColAttrPane
extends CFGridPane
implements ICFBamJavaFXDateColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDateColObj cur = getJavaFXFocusAsDateCol();
				if( cur != null ) {
					ICFBamDateColEditObj editObj = (ICFBamDateColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDateColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDateColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDateColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDateColObj getEffJavaFXFocus() {
		ICFBamDateColObj eff = (ICFBamDateColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDateColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDateColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDateColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDateColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDateColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDateColObj)editObj.create();
