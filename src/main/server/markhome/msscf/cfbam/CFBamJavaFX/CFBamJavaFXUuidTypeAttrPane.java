// Description: Java 11 JavaFX Attribute Pane implementation for UuidType.
...
/**
 *	CFBamJavaFXUuidTypeAttrPane JavaFX Attribute Pane implementation
 *	for UuidType.
 */
public class CFBamJavaFXUuidTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXUuidTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUuidTypeObj cur = getJavaFXFocusAsUuidType();
				if( cur != null ) {
					ICFBamUuidTypeEditObj editObj = (ICFBamUuidTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUuidTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXUuidTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUuidTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamUuidTypeObj getEffJavaFXFocus() {
		ICFBamUuidTypeObj eff = (ICFBamUuidTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUuidTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUuidTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		if( focus != null ) {
			editObj = (ICFBamUuidTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUuidTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUuidTypeObj)editObj.create();
