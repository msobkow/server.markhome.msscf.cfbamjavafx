// Description: Java 11 JavaFX View/Edit Form implementation for BoolType.
...
/**
 *	CFBamJavaFXBoolTypeViewEditForm JavaFX View/Edit Form implementation
 *	for BoolType.
 */
public class CFBamJavaFXBoolTypeViewEditForm
extends CFBorderPane
implements ICFBamJavaFXBoolTypePaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit BoolType";
	...
	public CFBamJavaFXBoolTypeViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBoolTypeObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamBoolTypeObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getBoolTypeFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamBoolTypeEditObj editObj = (ICFBamBoolTypeEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXBoolTypePaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamBoolTypeObj argFocus = (ICFBamBoolTypeObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamBoolTypeObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsBoolType() != null ) {
							if( null == getJavaFXFocusAsBoolType().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsBoolType();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXBoolTypePaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXBoolTypePaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
