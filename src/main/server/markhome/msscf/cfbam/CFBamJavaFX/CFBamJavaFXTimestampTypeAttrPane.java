// Description: Java 11 JavaFX Attribute Pane implementation for TimestampType.
...
/**
 *	CFBamJavaFXTimestampTypeAttrPane JavaFX Attribute Pane implementation
 *	for TimestampType.
 */
public class CFBamJavaFXTimestampTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTimestampTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTimestampTypeObj cur = getJavaFXFocusAsTimestampType();
				if( cur != null ) {
					ICFBamTimestampTypeEditObj editObj = (ICFBamTimestampTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTimestampTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTimestampTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimestampTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTimestampTypeObj getEffJavaFXFocus() {
		ICFBamTimestampTypeObj eff = (ICFBamTimestampTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTimestampTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTimestampTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTimestampTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTimestampTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTimestampTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTimestampTypeObj)editObj.create();
