// Description: Java 11 JavaFX Attribute Pane implementation for BoolDef.
...
/**
 *	CFBamJavaFXBoolDefAttrPane JavaFX Attribute Pane implementation
 *	for BoolDef.
 */
public class CFBamJavaFXBoolDefAttrPane
extends CFGridPane
implements ICFBamJavaFXBoolDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamBoolDefObj cur = getJavaFXFocusAsBoolDef();
				if( cur != null ) {
					ICFBamBoolDefEditObj editObj = (ICFBamBoolDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamBoolDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXBoolDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBoolDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsBoolDef( argFocus );
	...
	public ICFBamBoolDefObj getEffJavaFXFocus() {
		ICFBamBoolDefObj eff = (ICFBamBoolDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamBoolDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamBoolDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamBoolDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamBoolDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamBoolDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamBoolDefObj)editObj.create();
