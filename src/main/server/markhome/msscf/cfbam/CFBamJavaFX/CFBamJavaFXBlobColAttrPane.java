// Description: Java 11 JavaFX Attribute Pane implementation for BlobCol.
...
/**
 *	CFBamJavaFXBlobColAttrPane JavaFX Attribute Pane implementation
 *	for BlobCol.
 */
public class CFBamJavaFXBlobColAttrPane
extends CFGridPane
implements ICFBamJavaFXBlobColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamBlobColObj cur = getJavaFXFocusAsBlobCol();
				if( cur != null ) {
					ICFBamBlobColEditObj editObj = (ICFBamBlobColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamBlobColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXBlobColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBlobColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamBlobColObj getEffJavaFXFocus() {
		ICFBamBlobColObj eff = (ICFBamBlobColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamBlobColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamBlobColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamBlobColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamBlobColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamBlobColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamBlobColObj)editObj.create();
