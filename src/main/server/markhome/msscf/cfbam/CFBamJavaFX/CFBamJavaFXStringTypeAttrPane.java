// Description: Java 11 JavaFX Attribute Pane implementation for StringType.
...
/**
 *	CFBamJavaFXStringTypeAttrPane JavaFX Attribute Pane implementation
 *	for StringType.
 */
public class CFBamJavaFXStringTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXStringTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamStringTypeObj cur = getJavaFXFocusAsStringType();
				if( cur != null ) {
					ICFBamStringTypeEditObj editObj = (ICFBamStringTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamStringTypeObj focus = getEffJavaFXFocus();
	...
	protected class MaxLenCFLabel
		extends CFLabel
	{
		public MaxLenCFLabel() {
			super();
			setText( "Max. Length" );
		}
	}

	protected class MaxLenEditor
		extends CFInt32Editor
	{
		public MaxLenEditor() {
			super();
			setMinValue( CFBamStringDefBuff.MAXLEN_MIN_VALUE );
			setMaxValue( CFBamStringDefBuff.MAXLEN_MAX_VALUE );
			setFieldName( "Max. Length" );
		}
	}

	protected class IsPolymorphCFLabel
		extends CFLabel
	{
		public IsPolymorphCFLabel() {
			super();
			setText( "Is Polymorphic" );
		}
	}

	protected class IsPolymorphEditor
		extends CFBoolEditor
	{
		public IsPolymorphEditor() {
			super();
			setIsNullable( false );
			setFieldName( "Is Polymorphic" );
		}
	}
	...
	protected MaxLenCFLabel javafxLabelMaxLen = null;
	protected MaxLenEditor javafxEditorMaxLen = null;
	protected IsPolymorphCFLabel javafxLabelIsPolymorph = null;
	protected IsPolymorphEditor javafxEditorIsPolymorph = null;
	protected InitValueCFLabel javafxLabelInitValue = null;
	protected InitValueEditor javafxEditorInitValue = null;

	public CFBamJavaFXStringTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamStringTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		label = getJavaFXLabelIsPolymorph();
		setHalignment( label, HPos.LEFT );
		setValignment( label, VPos.BOTTOM );
		add( label, 0, gridRow );
		gridRow ++;

		ctrl = getJavaFXEditorIsPolymorph();
		setHalignment( ctrl, HPos.LEFT );
		add( ctrl, 0, gridRow );
		gridRow ++;
	...
	public ICFBamStringTypeObj getEffJavaFXFocus() {
		ICFBamStringTypeObj eff = (ICFBamStringTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamStringTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public MaxLenCFLabel getJavaFXLabelMaxLen() {
		if( javafxLabelMaxLen == null ) {
			javafxLabelMaxLen = new MaxLenCFLabel();
		}
		return( javafxLabelMaxLen );
	}

	public void setJavaFXLabelMaxLen( MaxLenCFLabel value ) {
		javafxLabelMaxLen = value;
	}

	public MaxLenEditor getJavaFXEditorMaxLen() {
		if( javafxEditorMaxLen == null ) {
			javafxEditorMaxLen = new MaxLenEditor();
		}
		return( javafxEditorMaxLen );
	}

	public void setJavaFXEditorMaxLen( MaxLenEditor value ) {
		javafxEditorMaxLen = value;
	}

	public IsPolymorphCFLabel getJavaFXLabelIsPolymorph() {
		if( javafxLabelIsPolymorph == null ) {
			javafxLabelIsPolymorph = new IsPolymorphCFLabel();
		}
		return( javafxLabelIsPolymorph );
	}

	public void setJavaFXLabelIsPolymorph( IsPolymorphCFLabel value ) {
		javafxLabelIsPolymorph = value;
	}

	public IsPolymorphEditor getJavaFXEditorIsPolymorph() {
		if( javafxEditorIsPolymorph == null ) {
			javafxEditorIsPolymorph = new IsPolymorphEditor();
		}
		return( javafxEditorIsPolymorph );
	}

	public void setJavaFXEditorIsPolymorph( IsPolymorphEditor value ) {
		javafxEditorIsPolymorph = value;
	}
	...
	{
		ICFBamStringTypeObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorMaxLen().setInt32Value( null );
		}
		else {
			getJavaFXEditorMaxLen().setInt32Value( popObj.getRequiredMaxLen() );
		}

		if( popObj == null ) {
			getJavaFXEditorIsPolymorph().setBooleanValue( null );
		}
		else {
			getJavaFXEditorIsPolymorph().setBooleanValue( popObj.getRequiredIsPolymorph() );
		}
	...
	public void postFields()
	{
		...
		ICFBamStringTypeObj focus = getJavaFXFocusAsStringType();
		ICFBamStringTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamStringTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setRequiredMaxLen( getJavaFXEditorMaxLen().getInt32Value() );

		editObj.setRequiredIsPolymorph( getJavaFXEditorIsPolymorph().getBooleanValue() );
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamStringTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamStringTypeObj)editObj.create();
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxEditorMaxLen != null ) {
			javafxEditorMaxLen.setDisable( ! isEditing );
		}
		if( javafxEditorIsPolymorph != null ) {
			javafxEditorIsPolymorph.setDisable( ! isEditing );
		}
		if( javafxEditorInitValue != null ) {
