// Description: Java 11 JavaFX View/Edit Form implementation for DateDef.
...
/**
 *	CFBamJavaFXDateDefViewEditForm JavaFX View/Edit Form implementation
 *	for DateDef.
 */
public class CFBamJavaFXDateDefViewEditForm
extends CFBorderPane
implements ICFBamJavaFXDateDefPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit DateDef";
	...
	public CFBamJavaFXDateDefViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDateDefObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamDateDefObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getDateDefFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void forceCancelAndClose() {
		...
		if( focus != null ) {
			ICFBamDateDefEditObj editObj = (ICFBamDateDefEditObj)focus.getEdit();
			if( editObj != null ) {
				if( editObj.getIsNew() ) {
					editObj.endEdit();
					editObj = null;
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXDateDefPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamDateDefObj argFocus = (ICFBamDateDefObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamDateDefObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsDateDef() != null ) {
							if( null == getJavaFXFocusAsDateDef().getEdit() ) {
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXDateDefPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXDateDefPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
