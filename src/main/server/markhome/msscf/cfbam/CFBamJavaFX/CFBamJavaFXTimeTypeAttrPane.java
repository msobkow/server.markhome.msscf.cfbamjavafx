// Description: Java 11 JavaFX Attribute Pane implementation for TimeType.
...
/**
 *	CFBamJavaFXTimeTypeAttrPane JavaFX Attribute Pane implementation
 *	for TimeType.
 */
public class CFBamJavaFXTimeTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTimeTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTimeTypeObj cur = getJavaFXFocusAsTimeType();
				if( cur != null ) {
					ICFBamTimeTypeEditObj editObj = (ICFBamTimeTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTimeTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTimeTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTimeTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTimeType( argFocus );
	...
	public ICFBamTimeTypeObj getEffJavaFXFocus() {
		ICFBamTimeTypeObj eff = (ICFBamTimeTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTimeTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTimeTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTimeTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTimeTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTimeTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTimeTypeObj)editObj.create();
