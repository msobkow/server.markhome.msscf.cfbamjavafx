// Description: Java 11 JavaFX Attribute Pane implementation for TZTimeCol.
...
/**
 *	CFBamJavaFXTZTimeColAttrPane JavaFX Attribute Pane implementation
 *	for TZTimeCol.
 */
public class CFBamJavaFXTZTimeColAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimeColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimeColObj cur = getJavaFXFocusAsTZTimeCol();
				if( cur != null ) {
					ICFBamTZTimeColEditObj editObj = (ICFBamTZTimeColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimeColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimeColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimeColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZTimeColObj getEffJavaFXFocus() {
		ICFBamTZTimeColObj eff = (ICFBamTZTimeColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimeColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimeColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimeColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimeColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZTimeColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimeColObj)editObj.create();
