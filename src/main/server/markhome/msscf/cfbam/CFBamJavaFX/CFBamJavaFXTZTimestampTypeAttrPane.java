// Description: Java 11 JavaFX Attribute Pane implementation for TZTimestampType.
...
/**
 *	CFBamJavaFXTZTimestampTypeAttrPane JavaFX Attribute Pane implementation
 *	for TZTimestampType.
 */
public class CFBamJavaFXTZTimestampTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimestampTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimestampTypeObj cur = getJavaFXFocusAsTZTimestampType();
				if( cur != null ) {
					ICFBamTZTimestampTypeEditObj editObj = (ICFBamTZTimestampTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimestampTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimestampTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimestampTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZTimestampTypeObj getEffJavaFXFocus() {
		ICFBamTZTimestampTypeObj eff = (ICFBamTZTimestampTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimestampTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimestampTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimestampTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimestampTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZTimestampTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimestampTypeObj)editObj.create();
