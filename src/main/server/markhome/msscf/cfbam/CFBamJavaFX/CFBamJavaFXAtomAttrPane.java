// Description: Java 11 JavaFX Attribute Pane implementation for Atom.
...
/**
 *	CFBamJavaFXAtomAttrPane JavaFX Attribute Pane implementation
 *	for Atom.
 */
public class CFBamJavaFXAtomAttrPane
extends CFGridPane
implements ICFBamJavaFXAtomPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamAtomObj cur = getJavaFXFocusAsAtom();
				if( cur != null ) {
					ICFBamAtomEditObj editObj = (ICFBamAtomEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamAtomObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXAtomAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamAtomObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamAtomObj getEffJavaFXFocus() {
		ICFBamAtomObj eff = (ICFBamAtomObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamAtomObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamAtomObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamAtomEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamAtomEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamAtomObj)editObj.create();
