// Description: Java 11 JavaFX Attribute Pane implementation for DoubleCol.
...
/**
 *	CFBamJavaFXDoubleColAttrPane JavaFX Attribute Pane implementation
 *	for DoubleCol.
 */
public class CFBamJavaFXDoubleColAttrPane
extends CFGridPane
implements ICFBamJavaFXDoubleColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDoubleColObj cur = getJavaFXFocusAsDoubleCol();
				if( cur != null ) {
					ICFBamDoubleColEditObj editObj = (ICFBamDoubleColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDoubleColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDoubleColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDoubleColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsDoubleCol( argFocus );
	...
	public ICFBamDoubleColObj getEffJavaFXFocus() {
		ICFBamDoubleColObj eff = (ICFBamDoubleColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDoubleColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDoubleColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDoubleColObj focus = getJavaFXFocusAsDoubleCol();
		ICFBamDoubleColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDoubleColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDoubleColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDoubleColObj)editObj.create();
