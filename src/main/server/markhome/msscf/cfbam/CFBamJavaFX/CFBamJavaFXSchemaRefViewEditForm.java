// Description: Java 11 JavaFX View/Edit Form implementation for SchemaRef.
...
/**
 *	CFBamJavaFXSchemaRefViewEditForm JavaFX View/Edit Form implementation
 *	for SchemaRef.
 */
public class CFBamJavaFXSchemaRefViewEditForm
extends CFBorderPane
implements ICFBamJavaFXSchemaRefPaneCommon,
	ICFForm
{
	public static String S_FormName = "View/Edit SchemaRef";
	...
	public CFBamJavaFXSchemaRefViewEditForm( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamSchemaRefObj argFocus, ICFFormClosedCallback closeCallback, boolean cameFromAdd ) {
		super();
		...
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamSchemaRefObj)argFocus.read( true );
			javaFXFocus = argFocus;
		}
		javafxViewEditPane = argSchema.getSchemaRefFactory().newViewEditPane( cfFormManager, argFocus );
	...
	public void setJavaFXFocus( ICFLibAnyObj value ) {
		...
		((ICFBamJavaFXSchemaRefPaneCommon)javafxViewEditPane).setJavaFXFocus( value );
		ICFBamSchemaRefObj argFocus = (ICFBamSchemaRefObj)value;
		if( ( argFocus != null ) && ( ! argFocus.getIsNew() ) ) {
			argFocus = (ICFBamSchemaRefObj)argFocus.read( true );
		...
		if( hboxMenu == null ) {
			...
			buttonEdit.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						if( getJavaFXFocusAsSchemaRef() != null ) {
							if( null == getJavaFXFocusAsSchemaRef().getEdit() ) {
			...
			buttonClose.setOnAction( new EventHandler<ActionEvent>() {
				@Override public void handle( ActionEvent e ) {
					...
					try {
						...
						cfFormManager.closeCurrentForm();
						if( formClosedCallback != null ) {
							focus = getJavaFXFocusAsSchemaRef();
	...
	public void setPaneMode( CFPane.PaneMode value ) {
		...
		try {
			...
			((ICFBamJavaFXSchemaRefPaneCommon)javafxViewEditPane).setPaneMode( value );
		}
		catch( Throwable t ) {
			super.setPaneMode( oldMode );
			((ICFBamJavaFXSchemaRefPaneCommon)javafxViewEditPane).setPaneMode( oldMode );
			throw t;
		}
		adjustButtons();
	}
