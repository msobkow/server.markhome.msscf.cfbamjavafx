// Description: Java 11 JavaFX Attribute Pane implementation for PopSubDep3.
...
/**
 *	CFBamJavaFXPopSubDep3AttrPane JavaFX Attribute Pane implementation
 *	for PopSubDep3.
 */
public class CFBamJavaFXPopSubDep3AttrPane
extends CFGridPane
implements ICFBamJavaFXPopSubDep3PaneCommon
{
	...
	implements ICFBamJavaFXRelationChosen
	{
		...
		public void choseRelation( ICFBamRelationObj value ) {
			if( javafxReferenceLookupRelation != null ) {
				ICFBamPopSubDep3Obj cur = getJavaFXFocusAsPopSubDep3();
				if( cur != null ) {
					ICFBamPopSubDep3EditObj editObj = (ICFBamPopSubDep3EditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamPopSubDep3Obj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXPopSubDep3AttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopSubDep3Obj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamPopSubDep3Obj getEffJavaFXFocus() {
		ICFBamPopSubDep3Obj eff = (ICFBamPopSubDep3Obj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamPopSubDep3Obj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamPopSubDep3Obj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		if( focus != null ) {
			editObj = (ICFBamPopSubDep3EditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamPopSubDep3EditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamPopSubDep3Obj)editObj.create();
