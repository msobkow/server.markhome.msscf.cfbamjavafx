// Description: Java 11 JavaFX Attribute Pane implementation for TZDateCol.
...
/**
 *	CFBamJavaFXTZDateColAttrPane JavaFX Attribute Pane implementation
 *	for TZDateCol.
 */
public class CFBamJavaFXTZDateColAttrPane
extends CFGridPane
implements ICFBamJavaFXTZDateColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZDateColObj cur = getJavaFXFocusAsTZDateCol();
				if( cur != null ) {
					ICFBamTZDateColEditObj editObj = (ICFBamTZDateColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZDateColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZDateColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZDateColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsTZDateCol( argFocus );
	...
	public ICFBamTZDateColObj getEffJavaFXFocus() {
		ICFBamTZDateColObj eff = (ICFBamTZDateColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZDateColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZDateColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZDateColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZDateColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZDateColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZDateColObj)editObj.create();
