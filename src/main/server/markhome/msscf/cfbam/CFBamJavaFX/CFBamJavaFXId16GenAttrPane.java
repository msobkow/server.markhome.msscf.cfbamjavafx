// Description: Java 11 JavaFX Attribute Pane implementation for Id16Gen.
...
/**
 *	CFBamJavaFXId16GenAttrPane JavaFX Attribute Pane implementation
 *	for Id16Gen.
 */
public class CFBamJavaFXId16GenAttrPane
extends CFGridPane
implements ICFBamJavaFXId16GenPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamId16GenObj cur = getJavaFXFocusAsId16Gen();
				if( cur != null ) {
					ICFBamId16GenEditObj editObj = (ICFBamId16GenEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamId16GenObj focus = getEffJavaFXFocus();
	...
	protected class CallbackDispenserChosen
	implements ICFBamJavaFXTableChosen
	{
		public CallbackDispenserChosen() {
		}

		public void choseTable( ICFBamTableObj value ) {
			if( javafxReferenceLookupDispenser != null ) {
				ICFBamId16GenObj cur = getJavaFXFocusAsId16Gen();
				if( cur != null ) {
					ICFBamId16GenEditObj editObj = (ICFBamId16GenEditObj)cur.getEdit();
					if( null != editObj ) {
						CFPane.PaneMode curMode = getPaneMode();
						if( ( curMode == CFPane.PaneMode.Add ) || ( curMode == CFPane.PaneMode.Edit ) ) {
							javafxReferenceLookupDispenser.setReferencedObject( value );
							editObj.setOptionalLookupDispenser( value );
						}
					}
				}
			}
	...
	protected class DispenserReferenceCallback
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			final String S_ProcName = "chose";
			...
			ICFBamSchemaObj schemaObj = (ICFBamSchemaObj)javafxSchema.getSchema();
			ICFBamId16GenObj focus = getEffJavaFXFocus();
			ICFBamTableObj referencedObj = (ICFBamTableObj)javafxReferenceLookupDispenser.getReferencedObject();
			java.util.List<ICFBamTableObj> listOfTable = null;
			Collection<ICFBamTableObj> cltn = null;
			CFBorderPane form = javafxSchema.getTableFactory().newPickerForm( cfFormManager, referencedObj, null, cltn, new CallbackDispenserChosen() );
	...
	protected class DispenserCFReferenceEditor
		extends CFReferenceEditor
	{
		public DispenserCFReferenceEditor() {
			super( new DispenserReferenceCallback() );
			setFieldName( "Dispensing Table" );
		}
	}
		...
		extends CFInt16Editor
	{
		public InitValueEditor() {
			super();
			setMinValue( CFBamInt16DefBuff.INITVALUE_MIN_VALUE );
			setMaxValue( CFBamInt16DefBuff.INITVALUE_MAX_VALUE );
		...
		extends CFInt16Editor
	{
		public MinValueEditor() {
			super();
			setMinValue( CFBamInt16DefBuff.MINVALUE_MIN_VALUE );
			setMaxValue( CFBamInt16DefBuff.MINVALUE_MAX_VALUE );
		...
		extends CFInt16Editor
	{
		public MaxValueEditor() {
			super();
			setMinValue( CFBamInt16DefBuff.MAXVALUE_MIN_VALUE );
			setMaxValue( CFBamInt16DefBuff.MAXVALUE_MAX_VALUE );
		...
		extends CFInt16Editor
	{
		public SliceEditor() {
			super();
			setMinValue( CFBamId16GenBuff.SLICE_MIN_VALUE );
			setMaxValue( CFBamId16GenBuff.SLICE_MAX_VALUE );
			setFieldName( "Slice" );
		}
	}
		...
		extends CFInt16Editor
	{
		public BlockSizeEditor() {
			super();
			setMinValue( CFBamId16GenBuff.BLOCKSIZE_MIN_VALUE );
			setMaxValue( CFBamId16GenBuff.BLOCKSIZE_MAX_VALUE );
	...
	protected DispenserCFReferenceEditor javafxReferenceLookupDispenser = null;
	...
	public CFBamJavaFXId16GenAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamId16GenObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		reference = getJavaFXReferenceLookupDispenser();
		setHalignment( reference, HPos.LEFT );
		add( reference, 0, gridRow );
		gridRow ++;
	...
	public ICFBamId16GenObj getEffJavaFXFocus() {
		ICFBamId16GenObj eff = (ICFBamId16GenObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamId16GenObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	public CFReferenceEditor getJavaFXReferenceLookupDispenser() {
		if( javafxReferenceLookupDispenser == null ) {
			javafxReferenceLookupDispenser = new DispenserCFReferenceEditor();
		}
		return( javafxReferenceLookupDispenser );
	}

	public void setJavaFXReferenceLookupDispenser( DispenserCFReferenceEditor value ) {
		javafxReferenceLookupDispenser = value;
	}
	...
	{
		ICFBamId16GenObj popObj = getEffJavaFXFocus();
		...
		if( javafxReferenceLookupDispenser != null ) {
			javafxReferenceLookupDispenser.setReferencedObject( javafxLookupDispenserObj );
		}
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setInt16Value( null );
		}
		else {
			getJavaFXEditorInitValue().setInt16Value( popObj.getOptionalInitValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMinValue().setInt16Value( null );
		}
		else {
			getJavaFXEditorMinValue().setInt16Value( popObj.getOptionalMinValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMaxValue().setInt16Value( null );
		}
		else {
			getJavaFXEditorMaxValue().setInt16Value( popObj.getOptionalMaxValue() );
		}
		...
		if( popObj == null ) {
			getJavaFXEditorBlockSize().setInt16Value( null );
		}
		else {
			getJavaFXEditorBlockSize().setInt16Value( popObj.getRequiredBlockSize() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamId16GenObj focus = getJavaFXFocusAsId16Gen();
		ICFBamId16GenEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamId16GenEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		javafxLookupDispenserObj = (ICFBamTableObj)( javafxReferenceLookupDispenser.getReferencedObject() );
		...
		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getInt16Value() );

		editObj.setOptionalMinValue( getJavaFXEditorMinValue().getInt16Value() );

		editObj.setOptionalMaxValue( getJavaFXEditorMaxValue().getInt16Value() );

		editObj.setRequiredSlice( getJavaFXEditorSlice().getInt16Value() );

		editObj.setRequiredBlockSize( getJavaFXEditorBlockSize().getInt16Value() );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamId16GenEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamId16GenObj)editObj.create();
	...
	public void adjustComponentEnableStates() {
		...
		if( javafxReferenceLookupDispenser != null ) {
			javafxReferenceLookupDispenser.setCustomDisable( ! isEditing );
		}
		if( javafxEditorId != null ) {
