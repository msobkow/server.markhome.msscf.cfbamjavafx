// Description: Java 11 JavaFX Attribute Pane implementation for BoolCol.
...
/**
 *	CFBamJavaFXBoolColAttrPane JavaFX Attribute Pane implementation
 *	for BoolCol.
 */
public class CFBamJavaFXBoolColAttrPane
extends CFGridPane
implements ICFBamJavaFXBoolColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamBoolColObj cur = getJavaFXFocusAsBoolCol();
				if( cur != null ) {
					ICFBamBoolColEditObj editObj = (ICFBamBoolColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamBoolColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXBoolColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamBoolColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
		...
		setJavaFXFocusAsBoolCol( argFocus );
	...
	public ICFBamBoolColObj getEffJavaFXFocus() {
		ICFBamBoolColObj eff = (ICFBamBoolColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamBoolColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamBoolColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamBoolColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamBoolColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamBoolColEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamBoolColObj)editObj.create();
