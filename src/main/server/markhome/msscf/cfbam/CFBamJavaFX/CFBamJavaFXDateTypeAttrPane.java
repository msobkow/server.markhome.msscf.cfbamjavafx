// Description: Java 11 JavaFX Attribute Pane implementation for DateType.
...
/**
 *	CFBamJavaFXDateTypeAttrPane JavaFX Attribute Pane implementation
 *	for DateType.
 */
public class CFBamJavaFXDateTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDateTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDateTypeObj cur = getJavaFXFocusAsDateType();
				if( cur != null ) {
					ICFBamDateTypeEditObj editObj = (ICFBamDateTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDateTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDateTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDateTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDateTypeObj getEffJavaFXFocus() {
		ICFBamDateTypeObj eff = (ICFBamDateTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDateTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDateTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDateTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDateTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDateTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDateTypeObj)editObj.create();
