// Description: Java 11 JavaFX Attribute Pane implementation for TZTimeDef.
...
/**
 *	CFBamJavaFXTZTimeDefAttrPane JavaFX Attribute Pane implementation
 *	for TZTimeDef.
 */
public class CFBamJavaFXTZTimeDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimeDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimeDefObj cur = getJavaFXFocusAsTZTimeDef();
				if( cur != null ) {
					ICFBamTZTimeDefEditObj editObj = (ICFBamTZTimeDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimeDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimeDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimeDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZTimeDefObj getEffJavaFXFocus() {
		ICFBamTZTimeDefObj eff = (ICFBamTZTimeDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimeDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimeDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimeDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimeDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTZTimeDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimeDefObj)editObj.create();
