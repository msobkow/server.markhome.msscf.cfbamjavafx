// Description: Java 11 JavaFX Attribute Pane implementation for TZTimestampCol.
...
/**
 *	CFBamJavaFXTZTimestampColAttrPane JavaFX Attribute Pane implementation
 *	for TZTimestampCol.
 */
public class CFBamJavaFXTZTimestampColAttrPane
extends CFGridPane
implements ICFBamJavaFXTZTimestampColPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTZTimestampColObj cur = getJavaFXFocusAsTZTimestampCol();
				if( cur != null ) {
					ICFBamTZTimestampColEditObj editObj = (ICFBamTZTimestampColEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTZTimestampColObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTZTimestampColAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTZTimestampColObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTZTimestampColObj getEffJavaFXFocus() {
		ICFBamTZTimestampColObj eff = (ICFBamTZTimestampColObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTZTimestampColObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTZTimestampColObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTZTimestampColEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTZTimestampColEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTZTimestampColObj)editObj.create();
