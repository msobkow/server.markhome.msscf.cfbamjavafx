// Description: Java 11 JavaFX Attribute Pane implementation for PopSubDep1.
...
/**
 *	CFBamJavaFXPopSubDep1AttrPane JavaFX Attribute Pane implementation
 *	for PopSubDep1.
 */
public class CFBamJavaFXPopSubDep1AttrPane
extends CFGridPane
implements ICFBamJavaFXPopSubDep1PaneCommon
{
	...
	implements ICFBamJavaFXRelationChosen
	{
		...
		public void choseRelation( ICFBamRelationObj value ) {
			if( javafxReferenceLookupRelation != null ) {
				ICFBamPopSubDep1Obj cur = getJavaFXFocusAsPopSubDep1();
				if( cur != null ) {
					ICFBamPopSubDep1EditObj editObj = (ICFBamPopSubDep1EditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamPopSubDep1Obj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXPopSubDep1AttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamPopSubDep1Obj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamPopSubDep1Obj getEffJavaFXFocus() {
		ICFBamPopSubDep1Obj eff = (ICFBamPopSubDep1Obj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamPopSubDep1Obj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamPopSubDep1Obj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamPopSubDep1EditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamPopSubDep1EditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamPopSubDep1Obj)editObj.create();
