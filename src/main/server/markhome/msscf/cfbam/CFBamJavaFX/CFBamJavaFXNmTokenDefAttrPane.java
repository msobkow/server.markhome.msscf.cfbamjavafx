// Description: Java 11 JavaFX Attribute Pane implementation for NmTokenDef.
...
/**
 *	CFBamJavaFXNmTokenDefAttrPane JavaFX Attribute Pane implementation
 *	for NmTokenDef.
 */
public class CFBamJavaFXNmTokenDefAttrPane
extends CFGridPane
implements ICFBamJavaFXNmTokenDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamNmTokenDefObj cur = getJavaFXFocusAsNmTokenDef();
				if( cur != null ) {
					ICFBamNmTokenDefEditObj editObj = (ICFBamNmTokenDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamNmTokenDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXNmTokenDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamNmTokenDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamNmTokenDefObj getEffJavaFXFocus() {
		ICFBamNmTokenDefObj eff = (ICFBamNmTokenDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamNmTokenDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamNmTokenDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamNmTokenDefObj focus = getJavaFXFocusAsNmTokenDef();
		ICFBamNmTokenDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamNmTokenDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamNmTokenDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamNmTokenDefObj)editObj.create();
