// Description: Java 11 JavaFX Attribute Pane implementation for DoubleType.
...
/**
 *	CFBamJavaFXDoubleTypeAttrPane JavaFX Attribute Pane implementation
 *	for DoubleType.
 */
public class CFBamJavaFXDoubleTypeAttrPane
extends CFGridPane
implements ICFBamJavaFXDoubleTypePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamDoubleTypeObj cur = getJavaFXFocusAsDoubleType();
				if( cur != null ) {
					ICFBamDoubleTypeEditObj editObj = (ICFBamDoubleTypeEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamDoubleTypeObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXDoubleTypeAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamDoubleTypeObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamDoubleTypeObj getEffJavaFXFocus() {
		ICFBamDoubleTypeObj eff = (ICFBamDoubleTypeObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamDoubleTypeObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamDoubleTypeObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamDoubleTypeObj focus = getJavaFXFocusAsDoubleType();
		ICFBamDoubleTypeEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamDoubleTypeEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamDoubleTypeEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamDoubleTypeObj)editObj.create();
