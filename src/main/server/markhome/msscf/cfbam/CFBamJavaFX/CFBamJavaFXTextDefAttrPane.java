// Description: Java 11 JavaFX Attribute Pane implementation for TextDef.
...
/**
 *	CFBamJavaFXTextDefAttrPane JavaFX Attribute Pane implementation
 *	for TextDef.
 */
public class CFBamJavaFXTextDefAttrPane
extends CFGridPane
implements ICFBamJavaFXTextDefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamTextDefObj cur = getJavaFXFocusAsTextDef();
				if( cur != null ) {
					ICFBamTextDefEditObj editObj = (ICFBamTextDefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamTextDefObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXTextDefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamTextDefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamTextDefObj getEffJavaFXFocus() {
		ICFBamTextDefObj eff = (ICFBamTextDefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamTextDefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamTextDefObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamTextDefObj focus = getJavaFXFocusAsTextDef();
		ICFBamTextDefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamTextDefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamTextDefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamTextDefObj)editObj.create();
