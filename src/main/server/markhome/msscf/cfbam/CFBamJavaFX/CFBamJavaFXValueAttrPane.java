// Description: Java 11 JavaFX Attribute Pane implementation for Value.
...
/**
 *	CFBamJavaFXValueAttrPane JavaFX Attribute Pane implementation
 *	for Value.
 */
public class CFBamJavaFXValueAttrPane
extends CFGridPane
implements ICFBamJavaFXValuePaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamValueObj cur = getJavaFXFocusAsValue();
				if( cur != null ) {
					ICFBamValueEditObj editObj = (ICFBamValueEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamValueObj focus = getEffJavaFXFocus();
	...
	public CFBamJavaFXValueAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamValueObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamValueObj getEffJavaFXFocus() {
		ICFBamValueObj eff = (ICFBamValueObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamValueObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamValueObj popObj = getEffJavaFXFocus();
	...
	public void postFields()
	{
		...
		ICFBamValueEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamValueEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamValueEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamValueObj)editObj.create();
