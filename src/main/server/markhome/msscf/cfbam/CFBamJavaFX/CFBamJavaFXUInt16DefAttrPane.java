// Description: Java 11 JavaFX Attribute Pane implementation for UInt16Def.
...
/**
 *	CFBamJavaFXUInt16DefAttrPane JavaFX Attribute Pane implementation
 *	for UInt16Def.
 */
public class CFBamJavaFXUInt16DefAttrPane
extends CFGridPane
implements ICFBamJavaFXUInt16DefPaneCommon
{
	...
	implements ICFBamJavaFXSchemaDefChosen
	{
		...
		public void choseSchemaDef( ICFBamSchemaDefObj value ) {
			if( javafxReferenceLookupDefSchema != null ) {
				ICFBamUInt16DefObj cur = getJavaFXFocusAsUInt16Def();
				if( cur != null ) {
					ICFBamUInt16DefEditObj editObj = (ICFBamUInt16DefEditObj)cur.getEdit();
					if( null != editObj ) {
	...
	implements ICFReferenceCallback
	{
		public void chose( ICFLibAnyObj value ) {
			...
			ICFBamUInt16DefObj focus = getEffJavaFXFocus();
		...
		extends CFUInt16Editor
	{
		public InitValueEditor() {
			super();
			setMinValue( CFBamUInt16DefBuff.INITVALUE_MIN_VALUE );
		...
		extends CFUInt16Editor
	{
		public MinValueEditor() {
			super();
			setMinValue( CFBamUInt16DefBuff.MINVALUE_MIN_VALUE );
		...
		extends CFUInt16Editor
	{
		public MaxValueEditor() {
			super();
			setMinValue( CFBamUInt16DefBuff.MAXVALUE_MIN_VALUE );
	...
	public CFBamJavaFXUInt16DefAttrPane( ICFFormManager formManager, ICFBamJavaFXSchema argSchema, ICFBamUInt16DefObj argFocus ) {
		super();
		Control ctrl;
		CFLabel label;
	...
	public ICFBamUInt16DefObj getEffJavaFXFocus() {
		ICFBamUInt16DefObj eff = (ICFBamUInt16DefObj)getJavaFXFocus();
		if( eff != null ) {
			if( null != eff.getEdit() ) {
				eff = (ICFBamUInt16DefObj)eff.getEdit();
			}
		}
		return( eff );
	}
	...
	{
		ICFBamUInt16DefObj popObj = getEffJavaFXFocus();
		...
		if( popObj == null ) {
			getJavaFXEditorInitValue().setUInt16Value( null );
		}
		else {
			getJavaFXEditorInitValue().setUInt16Value( popObj.getOptionalInitValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMinValue().setUInt16Value( null );
		}
		else {
			getJavaFXEditorMinValue().setUInt16Value( popObj.getOptionalMinValue() );
		}

		if( popObj == null ) {
			getJavaFXEditorMaxValue().setUInt16Value( null );
		}
		else {
			getJavaFXEditorMaxValue().setUInt16Value( popObj.getOptionalMaxValue() );
		}
	}
	...
	public void postFields()
	{
		...
		ICFBamUInt16DefObj focus = getJavaFXFocusAsUInt16Def();
		ICFBamUInt16DefEditObj editObj;
		if( focus != null ) {
			editObj = (ICFBamUInt16DefEditObj)(focus.getEdit());
		}
		else {
			editObj = null;
		}
		...
		editObj.setOptionalInitValue( getJavaFXEditorInitValue().getUInt16Value() );

		editObj.setOptionalMinValue( getJavaFXEditorMinValue().getUInt16Value() );

		editObj.setOptionalMaxValue( getJavaFXEditorMaxValue().getUInt16Value() );
	}
		...
		switch( value ) {
					...
					case View:
						if( editObj == null ) {
							if( focus != null ) {
								...
								editObj = (ICFBamUInt16DefEditObj)focus.beginEdit();
								if( editObj == null ) {
									throw new CFLibUsageException( getClass(),
										S_ProcName,
			...
			case Update:
				...
				if( editObj != null ) {
					postFields();
					if( editObj.getIsNew() ) {
						focus = (ICFBamUInt16DefObj)editObj.create();
